...
## Задача: APK logout не работи + аватар не се зарежда/избира (трети опит — май 2026)

**Проблем:** В APK приложението:
1. Logout бутонът е видим, кликва се, но след рестартиране на апликацията потребителят изглежда отново логнат
2. Аватарът не може да бъде избран (нито файловият пикер, нито камерата работят коректно)

**Коренни причини:**

### Причина 1 — `session-utils.js`: `getPreferencesPlugin()` не работи в iframe контекст
- Функцията проверяваше само `global.Capacitor` (= `window.Capacitor` в iframe)
- В Capacitor, Capacitor bridge-ът е инжектиран само в TOP window, не в iframes
- Резултат: `prefs = null` → `clearUserSessionData()` изчиства localStorage, но НЕ Capacitor Preferences
- При следващо стартиране: `native-backup.js` в index.html вижда празен localStorage и restore-ва всичко от Preferences → потребителят е "логнат" отново

### Причина 2 — `profile.html`: аватар upload click handler на LABEL, а не на INPUT
- File input е с `z-index:2` и покрива цялата аватар зона — кликовете кацат на INPUT-а
- `e.preventDefault()` се извикваше СЛЕД `getAvatarCamera()` check → ако камерата е налична, но не преди native file picker-ът да се отвори
- В Android WebView, native file picker се отваря преди bubble фазата на JS events → `e.preventDefault()` в bubble handler е твърде късно

**Поправки:**

### session-utils.js
- `getPreferencesPlugin()`: добавен fallback `global.top && global.top !== global && global.top.Capacitor`
- Идентично с вече работещия pattern в `native-backup.js` (`_getCap()`)
- Сега Capacitor Preferences се изчистват ПРАВИЛНО при logout от iframe

### profile.html
- В APK режим: `avatarInput.style.pointerEvents = 'none'` — кликовете минават директно до label-а (не до file input-а)
- `e.preventDefault()` се извиква ПРЕДИ `getAvatarCamera()` check — file picker НИКОГА не се отваря в APK
- Реда с `e.preventDefault()` е преместен да е първото нещо в APK click handler-а

**Засегнати файлове:** `session-utils.js`, `profile.html`, `logtasks.md`
