...
## 2026-05-21 — XBody back button + durable local restore

**Задача:** „Back бутонът в xbody.html да се свали с 3 мм надолу. Всички въведени данни и отметки в XBody формата да се пазят локално и да се възстановяват автоматично при бъдещи посещения… и все пак внимавай как го правиш, за да не влезеш в конфликт с основния сайт и да блокира всичко“

**Направено:**
- Свалих `#back-btn` в `xbody.html` с още `3mm` надолу (`5mm` → `8mm` над safe area offset-а).
- Върнах XBody iframe-а към same-origin proxy flow през `/schedule.php?...&xbodyProxy=1`, но само когато страницата вече е под контрола на `xbody-sw.js`; при първо зареждане остава безопасен fallback към директния Acuity URL.
- Разширих `xbody-sw.js` да proxy-ва само XBody заявките с `xbodyProxy=1`, правилния `owner` и `appointmentType`, за да няма страничен ефект върху основния сайт.
- Инжектирах локален persistence script в proxy-натия Acuity HTML, който пази и възстановява текстови полета, textarea, select, radio и checkbox стойности чрез `localStorage`, включително при динамични DOM промени.
- Умишлено оставих payment/card полетата извън локалното съхранение, за да не се записват чувствителни картови данни в браузъра.

**Резултат:** XBody формата вече възстановява трайно локално пазените неплатежни данни и отметки при бъдещи посещения, а proxy логиката е ограничена така, че да не засяга останалата част от сайта.
