...
## 2026-05-24 — Хаптик вибрацията не е синхронизирана с края на въвеждането в чата

**Задача:** Вибрацията на haptic функцията не е синхронизирана с реалния край на въвеждането на символи в чата.

**Причина (намерена):**
`startChatTypingHaptics(msgLen)` предварително изчислява и изпраща **цял pattern** на `navigator.vibrate()` за очакваната продължителност `N×18 + 300` ms. На мобилни устройства `setTimeout` има timer drift — реалното въвеждане отнема **повече** от изчисленото, затова вибрационният pattern свършва **преди** последния символ. Буферът от 300 ms компенсира само ~16 символа дрифт. При дълги отговори вибрацията спира значително преди края. Обратното — при `vibrate(0)` в `finalizeTyping()` за кратки съобщения, вибрацията продължава 300 ms след последния символ ако `vibrate(0)` не е достатъчно надежден на конкретния Android.

**Направено (plan.html):**
1. Премахната функция `startChatTypingHaptics` и нейното извикване преди `tick()`.
2. В `tick()` — добавено `pulseTypingHaptic(8, 'chat')` при всяко въвеждане на символ.
   - Всяка итерация изпраща кратък 8ms pulse, точно синхронизиран с текущия символ.
   - Вибрацията приключва естествено с последния символ.
   - `stopTypingHaptics()` в `finalizeTyping()` остава като допълнителна гаранция.

**Резултат:** Haptic вибрацията е точно синхронизирана с всеки въведен символ и спира едновременно с последния.
