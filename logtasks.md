...
## 2026-05-25 — Оптимизация: премахване на мъртъв код и debug logs

**Задача:** Намиране и премахване на обемния/тежкия, ненужен и мъртъв код в най-големите файлове на проекта.

**Анализирани файлове (по размер):**
- `worker.js` — 629KB / 13447 реда: предимно легитимен код (AI промпт генерация); само 3 debug logs, без мъртъв код
- `admin.html` — 656KB / 10141 реда
- `plan.html` — 494KB / 10374 реда
- `questionnaire2.html` — 355KB / 4881 реда
- `profile.html` — 255KB / 4997 реда
- `index.html` — 206KB / 4469 реда

**Направено:**
1. **Премахнати всички `if (PWA_DEBUG) {...}` блокове** от 5 файла — тъй като `PWA_DEBUG = false` навсякъде, тези блокове никога не изпълняват, но заемат код
2. **Премахнати `const PWA_DEBUG = false;` декларации** от всички файлове
3. **Премахнати debug `console.log` statements** от всички HTML файлове (не-гардирани debug logs: connection, dedup, notification, service worker registration, cache hit/miss)
4. **admin.html**: премахнати Cache HIT/MISS debug logs

**Резултат (541 реда премахнати):**
- `plan.html`: 10374 → 10274 реда, 494KB → 474KB
- `profile.html`: 4997 → 4900 реда, 255KB → 241KB
- `questionnaire2.html`: 4881 → 4784 реда, 355KB → 339KB
- `questionnaire.html`: 3136 → 3040 реда, 165KB → 153KB
- `index.html`: 4469 → 4332 реда, 206KB → 190KB
- `admin.html`: 10141 → 10127 реда, 656KB → 640KB
