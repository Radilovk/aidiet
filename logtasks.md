...
**Потвърдена причина:**
- В `plan.html` `_positionFabBelowBubble()` смяташе `bottom` спрямо долния край на `game-bubble-body`, но не изваждаше височината на самия FAB.
- Така кодът оставяше `FAB_BUBBLE_GAP` между bubble-а и долния край на FAB-а, вместо между bubble-а и горния край на FAB-а, което вкарва бутона под `game-bubble-body`.
- Допълнително измерването чрез `getBoundingClientRect()` попадаше върху активната `bubbleInTop` scale анимация и даваше временна трансформирана позиция вместо крайния layout размер.

**Направено:**
- `plan.html`: позиционирането вече ползва layout `offsetTop/offsetHeight`, за да не зависи от временния transform по време на bubble анимацията.
- `plan.html`: при изчислението вече се изважда и височината на FAB-а, така че реално да остава малък видим gap между `game-bubble-body` и чат бутона.

## 2026-05-31 — Chat FAB: плавно единично повдигане при prompt bubble

**Задача:** Да се намери реалната причина за резките двойни скокове на `fab-chat fab-raised` при показване на запитване и да се оправи с минимум код, като крайната позиция остане малко под долния край на `game-bubble-body`.

**Потвърдена причина:**
- `plan.html` комбинираше две отделни вертикални движения на FAB-а: `fabBounce` (`transform: translateY(...)` на няколко резки стъпки) и отделно промяна на `bottom` при `fab-raised`.
- След това същият FAB се позиционираше втори път чрез `_positionFabBelowBubble()`, защото първо се вдигаше към фиксирана междинна стойност, а после се местеше пак според реалния bubble размер.
- Bubble-ът първоначално се рендерираше с празен въпрос и нарастваше чак по време на typewriter ефекта, което правеше „финалната“ FAB позиция известна твърде късно и причиняваше второ преместване.

**Направено:**
- `plan.html`: махнато е bounce повдигането на `game-prompt`, за да не наслагва резки `translateY` скокове върху движението нагоре.
- `plan.html`: премахнато е фиксираното междинно `fab-raised` bottom позициониране; FAB вече се мести директно към измерената крайна позиция.
- `plan.html`: bubble-ът резервира финалната височина още преди typewriter анимацията и FAB се подравнява еднократно по `.game-bubble-body` с малък отстъп от 6px.
...
**Направено:**
- `local-scheduler.js`: добавен е минимален public helper `scheduleTestGameQuestionNotification(10)`, който планира еднократно `evening_check` известие след +10 сек (Capacitor first, SW fallback), без backend заявка.
- `plan.html`: добавена е чат команда `*notifyme`, която извиква helper-а и връща потвърждение с точния час на известяването.

## 2026-05-27 — pep.html: премахване на бутона за демо данни

**Задача:** В `pep.html` да се премахне бутонът за демо данни.

**Направено:**
- `pep.html`: премахнат е бутонът „Демо данни“ от панела с действия.
- `pep.html`: изчистени са свързаните `disabled` и click handler препратки, за да няма висящи елементи след премахването на бутона.

## 2026-05-27 — pep.html: XLS export, custom multiplier и preview преди запис

**Задача:** В `pep.html` CSV експортът да се смени с по-надежден файл, да се махне стандартната продажба ×2.2, да има възможност за ръчно попълване на множител, да има видим preview преди запис и да се премахне колоната „Стандартна продажба“ от продуктовата таблица.

**Направено:**
- `pep.html`: бутонът за експорт вече генерира `.xls` файл с Excel-съвместима таблица вместо CSV.
- `pep.html`: премахната е опцията „Стандартна продажба · ×2.2“ от формата и колоната `Стандарт ×2.2` от `productTable`.
- `pep.html`: добавен е `multiplierCustom` за персонален множител и записът използва въведения множител, когато е избрана custom опцията.
- `pep.html`: добавен е live preview преди запис, който показва продукта, количеството, множителя, крайната цена и доставната стойност.
...
**Направено:**
- `pep.html`: добавена е лека проверка за local/file контекст и при нея страницата зарежда директно локалните данни, без да опитва `/api/pep/bootstrap`.
- `pep.html`: при този път статусът се показва като `local-fallback`, без warning в конзолата.

## 2026-05-27 — pep.html: без шумен console.warn при fallback

**Задача:** Да се махне конзолният warning при fallback, защото страницата вече има нормален локален авариен режим.

**Направено:**
- `pep.html`: премахнат е `console.warn('PEP backend unavailable...')` от bootstrap fallback-а; потребителят вижда само инфо банер.
...
**Направено:**
- `pep.html`
  - Пълен UX/UI редизайн с по-ясна структура: hero секция, статус на синхронизацията, по-разбираема форма, по-добра статистика, търсене в историята и по-чист каталог.
  - Подменен е неясният термин „множител“ с по-разбираеми типове записи: продажба на едро, стандартна продажба, продажба на дребно и лична употреба.
  - Добавен е бекенд-first flow към `/api/pep/*`, като при недостъпен бекенд страницата преминава в локален авариен режим без да спира да работи.
- `worker.js`
  - Добавени са PEP API endpoints: bootstrap, добавяне на продукт, добавяне на продажба, изтриване на продажба и reset на демо данните.
  - Добавена е бекенд абстракция с приоритет Cloudflare D1 (`PEP_DB`), а при липса на D1 — fallback към съществуващото KV хранилище.
  - Демо продуктите и демо продажбите вече се seed-ват от бекенда вместо само от `localStorage`.
- `migrations/0001_pep_sales.sql`
  - Добавена е схема за `pep_products`, `pep_sales` и `pep_meta`.
- `wrangler.toml`
  - Добавен е коментиран шаблон за D1 binding на `PEP_DB`, за да може реалната база да се закачи без да се чупи текущата конфигурация.
...
**Направено:**
- `profile.html`: `social-logout-btn` е заменен със `spa-logout-btn` на същото място и е обновен стилът му с по-изчистена pill форма
- `index.html`: премахнат е старият shell `spa-logout-btn`, така че класът остава само в profile logout бутона
...
## 2026-06-01 — Промени в Анализ tab (game-analytics)

**Задача:** Три промени в game-analytics секцията:
1. Опростяване на game-index-hint — премахване на излишни думи, оставени само оценки (Отлично/Добре/Нисък и т.н.) + инфо икона.
2. Замяна на "Стремеж" с "Цел: минимум (X стойност)" в game-explain-text.
3. Добавяне на бутон "Защо е важно" в модалния прозорец с научна информация.

**Какво е направено:**
- `game-analytics.html`: Опростени 6 game-index-hint реда — премахнати "натисни за детайли", "По-висок % = по-добре", "Дневен % (хранения + чек-ини)" и др.
- `game-analytics.html`: EXPLANATIONS обект — всички "Стремеж: над X%" заменени с "Цел: минимум X%". Текстовете съкратени и ясни.
- `game-analytics.html`: Добавено `why` поле във всеки EXPLANATIONS запис с научна информация от реални проучвания.
- `game-analytics.html`: Нова функция `showWhyModal()` — модален прозорец за научната информация при натискане на "Защо е важно".
- `game-analytics.html`: CSS за `.game-explain-why-btn`.
- `profile.html`: Всички същи промени приложени (profile има копие на game-analytics кода).

## 2026-06-02 — Отстраняване на проблем с липсващи автоматични нотификации в APK

**Задача:** Да се разбере защо при изтеглено и инсталирано APK на NutriPlan не се извеждат автоматични нотификации на телефона и да се отстрани проблемът по най-минималистичен начин.

**Анализ:** 
- Приложението използва Capacitor plugin-а `@capacitor/local-notifications`. 
- В `local-scheduler.js` нотификациите се насрочват с настройка `allowWhileIdle: true`, за да не бъдат "убивани" от Doze режима на батерията. 
- Това кара Capacitor да използва Android API `setExactAndAllowWhileIdle()`. 
- На устройства с Android 14+ (API 34), разрешението за точни аларми (`SCHEDULE_EXACT_ALARM`) вече не се дава по подразбиране. Ако то липсва, опитът за насрочване хвърля `SecurityException` и нотификациите не се регистрират изобщо.
- Тъй като приложението се инсталира като изтеглено APK, най-доброто и минималистично решение е добавянето на разрешението `USE_EXACT_ALARM` в `AndroidManifest.xml`, което се дава автоматично от системата без да изисква действие от потребителя.
