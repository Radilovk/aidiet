...
**Направено:**
- Одитирах merge-а `#841` срещу pre-merge parent-а и потвърдих, че APK план restore, скриването на chat/image FAB в `index`, и game prompt позиционирането вече са върнати локално-first.
- Намерих остатъчен UX regression в `index.html`: `TAB_ACTIVATED` само извикваше `reveal()`, но не нулираше вече активираните `.reveal`/hero класове, затова анимациите не се преиграваха при следващо отваряне на Home таба.
- Поправих `index.html` с минимална промяна: при `TAB_ACTIVATED` Home табът ресетва hero/reveal класовете и ги пуска отново през `requestAnimationFrame`, без нови network/backend извиквания.
- След новия сигнал за APK проверих shell shortcut потока и подсилих `app.html`: chat/image FAB действията вече първо извикват директно `openChat()` / `openFoodAnalysis()` в same-origin `plan.html` iframe-а, а `postMessage` остава само като fallback. Това избягва WebView/APK проблеми с изгубени shortcut съобщения.

**Резултат:** Home/index анимациите отново се стартират всеки път при връщане към таба, като shell keep-alive поведението и ниското backend натоварване остават запазени.

## 2026-05-21 — APK plan/login, tab animations, index FAB cleanup

**Задача:** „В АПК след логване план не се зарежда; анимациите да се активират при всяко отваряне на таб; chat/image иконите да не са в index; game въпросите и chat асистентът да стоят горе до края; върни предишния UX/UI, намери несъответствия, оптимизирай с минимум код и минимално backend натоварване.“

**Направено:**
- Поправих `plan.js` да не блокира локално възстановен APK план, когато Firebase auth state не е наличен, но `dietPlan` вече е върнат от локален/native backup.
- Запазих backend щадящото поведение: не добавих нови заявки; промяната предпочита вече наличния локален план.
- Скрих shell chat/image FAB бутоните в `app.html`, когато активният таб е `home/index`.
- Добавих леко повторно shell tab enter движение, което се стартира при всяко активиране на таб.
- Добавих `TAB_ACTIVATED` refresh за reveal секциите в `index.html`.
- Свързах game prompt състоянието от `plan.js` към shell-а, така че chat иконата в APK shell се качва горе и остава там до края на опашката от въпроси.

**Резултат:** APK shell запазва keep-alive UX и ниско backend натоварване, планът може да се покаже от локален restore след login, home/index няма chat/image бутони, а tab/game анимациите се държат по-близо до предишния облик.
...
## 2026-05-21 — Анализ: функции преди vs. дефекти след обединяване в app.html

**Задача:** Направи пълен списък с всички функции, елементи, UX/UI, поведение от ПРЕДИ обединяването и ДЕФЕКТИТЕ след него.

**Направено:** Пълен одит на app.html, plan.html, guidelines.html, profile.html, plan.js, profile.js, guidelines.js, sw.js. Резултатът е предоставен директно в отговора (виж по-долу).
