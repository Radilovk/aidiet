...
## 2026-05-20 — Диагностика: Auto-транслатор не работи в xbody.html

**Задача:** Разбери защо Google Translate автоматичният превод не работи в xbody.html.

**Анализ (без промени по кода):**

Открити са 3 причини защо Google Translate не работи:

1. **Cross-origin iframe ограничение (основна причина):** Acuity формата е в `<iframe src="https://app.acuityscheduling.com/...">` — различен origin. Google Translate може да модифицира само DOM-а на текущата страница. Браузърната Same-Origin Policy напълно блокира достъпа до cross-origin iframe съдържание — Google Translate физически не може да преведе Acuity формата.

2. **`pageLanguage: 'en'` срещу `<html lang="bg">`:** Декларирано е, че страницата е на английски (`pageLanguage: 'en'`), но `<html lang="bg">` казва обратното. Статичните текстове на страницата са вече на български (Назад, Инсталирай сега, По-късно) — няма какво да се превежда.

3. **Правилното решение вече е приложено:** Acuity iframe-ът вече зарежда `&locale=bg` (ред 338) — единственият коректен начин за BG локализация на Acuity формата.

**Заключение:** Google Translate widget-ът в xbody.html е безполезен — може да се премахне (редове 593–618). Всичко работи коректно чрез `&locale=bg`.

---

## 2026-05-20 — Google Translate Proxy за Acuity iframe в xbody.html

**Задача:** Имплементирай Google Translate proxy (`translate.goog`) за Acuity Scheduling iframe, за да се постигне автоматичен EN→BG превод без backend и без нарушаване на layout-а.

**Направено:**

1. **Заменен iframe src** (ред 342): Оригиналният `https://app.acuityscheduling.com/schedule.php?owner=13943721&appointmentType=16859189&locale=bg` е заменен с Google Translate proxy URL:
   ```
   https://app-acuityscheduling-com.translate.goog/schedule.php?owner=13943721&appointmentType=16859189&_x_tr_sl=en&_x_tr_tl=bg&_x_tr_hl=bg&_x_tr_pto=wapp
   ```
   Домейнът `app.acuityscheduling.com` → `app-acuityscheduling-com.translate.goog` (точките стават тирета, прибавено `.translate.goog`).

2. **Премахнат мъртвият Google Translate widget** (бившите редове 593–618): `#google_translate_element` div, `googleTranslateElementInit()` функцията и CDN скриптът вече не са нужни — proxy подходът ги замества напълно.

**Следваща стъпка:** Ръчен тест на live транзакция (Stripe Elements) под `.translate.goog` домейна — проверка за CSP/Origin блокиране.
