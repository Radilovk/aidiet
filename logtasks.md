...
## 2026-05-22 — Анализ: един имейл не трябва да трупа много plan записи

**Задача:** Да се провери логиката за идентификация по имейл и да се изведат реално хванатите грешки и приложими решения, така че един имейл да не натрупва много client/plan записи.

**Реално хванати грешки:**
1. **Грешен primary identity при запис:** `questionnaire2.html` генерира нов случаен `clientId` при всяко попълване, вместо да използва стабилна email-базирана идентичност или email index.
2. **Няма dedupe/update при запис:** `worker.js` (`handleSaveClientData`) винаги записва нов `client:${clientId}` и винаги добавя `clientId` в `clients_list`, без проверка за съществуващ normalized email.
3. **Email matching е само късен workaround:** `handleGetUserProfile` сканира `clients_list` по имейл само когато търси activated plan; това не предотвратява натрупването на pending/duplicate записи и не е source of truth.
4. **Approval flow е вързан за clientId, не за email:** admin update/activate работят по конкретен `clientId`, което позволява стар/грешен запис за същия имейл да остане активен, а новият да виси pending.

**Приложими решения:**
1. **Въведи каноничен ключ по normalized email** (`trim().toLowerCase()`), напр. email→clientId index, или направо един каноничен client record на имейл.
2. **При повторно попълване със същия имейл:** update на съществуващия запис вместо create на нов; ако има нужда от история, тя да е в revisions/version history към същия запис, не в отделни client записи.
3. **При нов план за вече одобрен клиент:** обновяване на същия запис и reset към `pending` за ново одобрение (или auto-activate само ако това е изрично бизнес правило), но без втори client запис за имейла.
4. **`clients_list` да е dedupe-нат по каноничния client record**, за да няма множество редове за един и същ имейл в admin панела.
5. **Еднократна миграция/cleanup:** групиране на старите записи по normalized email, избор на каноничен запис, пренасочване на profile/client linkage към него и игнориране/архивиране на останалите.

**Резултат:** Потвърдено е, че проблемът е реален и е архитектурен — системата в момента идентифицира client record-а по случаен `clientId`, а не по имейл, затова за един имейл могат да се натрупват много plan/client записи.
