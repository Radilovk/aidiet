...
## 2026-05-25 — Хаптик в APK: усилване на native feedback

**Задача:** В APK хаптикът при typing се усеща много по-слабо от уеб/PWA. Трябваше да се проучи причината и да се усили без излишно раздуване.

**Причина:**
- В момента APK path-ът ползва `Haptics.impact({ style })`.
- По Android това е умишлено много фин haptic effect и често е по-слаб от `navigator.vibrate(...)` pattern-а, който браузърът изпълнява при PWA/уеб.
- Затова реално имаше работещ хаптик, но твърде слаб за този typing use case.

**Направено:**
1. **`plan.html`** — `hapticCtrl.trigger()` вече изчислява реална native продължителност (`12ms` за букви, `20ms` за пунктуация) и я подава към shell-а.
2. **`plan.html`** — standalone APK path-ът вече lazy-register-ва `Haptics` proxy-то и на Android предпочита `plugin.vibrate({ duration })` вместо `impact({ style })`; на iOS/други платформи запазва `impact`.
3. **`app.js`** — `NUTRIPLAN_HAPTIC` handler-ът в shell-а на Android вече използва `Haptics.vibrate({ duration })` вместо `impact`, а на останалите платформи остава `impact`.
4. **Проучване:** Проверих наличния код и публичната документация/обяснения за Capacitor/Android — `impact` на Android е по design по-фин от директна вибрация, затова за по-осезаем feedback е правилно да се мине към кратък `vibrate` duration.

**Резултат:** В APK typing хаптикът вече трябва да е осезаемо по-силен и по-близък до това, което се усеща в PWA/уеб, без да се пипат други части от приложението.
