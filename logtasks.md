...
## 2026-05-21 — XBody отваря NutriPlan вместо собствената си страница

**Задача:** Оправи проблема, при който при влизане/инсталиране на XBody се отваря NutriPlan вместо XBody.

**Причина:**
- `xbody-manifest.json` нямаше собствен `id` и `scope`, така че браузърът виждаше XBody като приложение от същия origin/scope като NutriPlan.
- `start_url` беше относителен, а XBody страницата ползваше и общите NutriPlan икони, което допълнително смесваше идентичността на двете инсталации.

**Направено:**
1. **`xbody-manifest.json`:**
   - Добавен е отделен `id: "/xbody.html"`.
   - `start_url` е направен абсолютен (`/xbody.html`).
   - Добавен е тесен `scope: "/xbody.html"`, за да остане XBody отделно от NutriPlan PWA.
2. **`xbody.html`:**
   - Подменени са favicon / apple-touch-icon / install modal иконите с `xbody-icon-*`, вместо общите NutriPlan икони.
3. **`xbody-sw.js`:**
   - Cache версията е bump-ната до `xbody-v7`.
   - Optional cache иконите са сменени към `xbody-icon-192.png` и `xbody-icon-512.png`.

**Очакван резултат:** XBody вече има собствена PWA идентичност и при отваряне/инсталиране не трябва да се слива с NutriPlan.
## 2026-05-21 — APK shell стабилизация: табове + chat/image shortcuts

**Задача:** Оправи описаните APK проблеми прецизно и с минимум разводняване: бавно първо превключване между табовете, разбъркване на табовете след relaunch, липсващи икони/бутони за чат и анализ на изображение, и намери директно свързани shell регресии.

**Потвърдени причини:**
- `app.html` зареждаше всички iframe табове веднага, което товари WebView още при първо влизане и прави първите превключвания по-тежки.
- Shell-ът винаги стартираше от `plan` без да пази последния активен таб, което правеше релончването непоследователно и създаваше усещане за „разбъркани“ табове.
- В shell режим `plan.html` / `guidelines.html` / `profile.html` нарочно крият `.fab-chat` и `.fab-food`, но `app.html` не рендерираше собствени заместители, затова shortcut-ите изчезваха напълно.
- Chat shortcut иконата `docs/Chat2.png` не беше в service worker static cache, което прави показването ѝ по-крехко при APK/PWA кеширане.

**Направено:**
1. **`app.html`:**
   - Оставен е `plan` табът eager, а `home/guidelines/profile` вече се warm-up-ват след first paint / idle вместо да товарят старта наведнъж.
   - Добавено е shell-level състояние за активния таб (`np_shell_active_tab`), за да се възстановява последният отворен таб при relaunch.
   - Добавени са shell-level floating shortcut бутони за чат и анализ на храна, така че да са видими и в iframe shell режима.
   - Добавен е `PLAN_SHORTCUT` bridge + pending action handling, за да се отвори директно chat/food overlay в `plan.html`, вместо само да се превключи към таба.
2. **`plan.html`:**
   - Embedded listener-ът вече обработва `PLAN_SHORTCUT` и извиква `openChat()` / `openFoodAnalysis()` директно.
3. **`sw.js`:**
   - Cache version bump `nutriplan-v12` → `nutriplan-v13`.
   - Добавен е `docs/Chat2.png` в static cache.

**Очакван резултат:** Първото APK влизане вече не товари едновременно всички тежки табове, активният таб се възстановява консистентно при повторно отваряне, а chat / image-analysis shortcut-ите отново се виждат и работят правилно в shell режима.
...
## 2026-05-21 - Задача: xbody.html back бутон + локално запазване на въведените данни

- **Задача:**
  1. Back бутонът в `xbody.html` да се свали с 3 мм надолу.
  2. Всички въведени данни и отметки в XBody формата да се пазят локално и да се възстановяват автоматично при бъдещи посещения.
- **Направено:**
  1. **xbody.html:** Позицията на `#back-btn` е преместена с още `3mm` надолу (`5mm` → `8mm` в `top` offset-а).
  2. **xbody.html:** Зареждането на Acuity iframe вече изчаква активен `xbody-sw.js` и при успех отваря same-origin proxy route `/schedule.php?...`; при липса на Service Worker остава fallback към директния Acuity URL.
  3. **xbody-sw.js:** Добавен е proxy handler за навигации към `/schedule.php`, който fetch-ва Acuity HTML, инжектира `<base href="https://app.acuityscheduling.com/">` и връща страницата като same-origin документ.
  4. **xbody-sw.js:** Инжектиран е persistence script, който пази в `localStorage` стойностите на текстови полета, `textarea`, `select`, `checkbox` и `radio` и ги възстановява автоматично при следващи посещения, включително след затваряне на страницата.
  5. **xbody-sw.js:** Добавени са `MutationObserver` и `input/change/pagehide` hooks, за да работи save/restore и при динамичните стъпки на Acuity SPA.
  6. **xbody-sw.js:** Чувствителни платежни полета (карта, CVV, expiry и подобни) умишлено не се записват в `localStorage`, за да не се въвежда риск за сигурността.
  7. **xbody-sw.js:** Bump на cache version `xbody-v5` → `xbody-v6`, за да се форсира update на PWA shell-а и новия proxy/persistence код.
