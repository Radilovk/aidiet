...
### Стъпка 1: Отворете страницата за възстановяване

Отворете в **същия браузър**, където са били попълнени въпросниците:
...
или локално:
```
questionnaire2-recovery.html
```

### Стъпка 2: Прегледайте намерените данни

Страницата автоматично ще:
- Потърси всички записи от Въпросник 2.0 в localStorage
- Покаже списък с намерените данни
- Показва основна информация: ID, дата, име, имейл, възраст, пол, брой файлове

### Стъпка 3: Запазете данните

Имате два избора:

#### Опция 1: Запазване в сървъра (Препоръчително)
1. Кликнете бутон "Запази в сървъра"
2. Данните се изпращат към API endpoint `/api/save-questionnaire2-data`
3. Записват се в KV хранилището на Cloudflare
4. След успешно запазване, се изтриват от localStorage
5. Получавате съобщение за успех/грешка

#### Опция 2: Изтегляне като файл
1. Кликнете бутон "Изтегли като файл"
2. Данните се изтеглят като JSON файл
3. Файлът може да бъде архивиран или обработен офлайн
4. Данните **НЕ** се изтриват от localStorage
