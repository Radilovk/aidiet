...
const ACUITY_OWNER = '13943721';
const ACUITY_APPOINTMENT_TYPE = '16859189';
const ACUITY_PROXY_FLAG = 'xbodyProxy';
...
function shouldProxyAcuity(url) {
  return url.pathname === ACUITY_PROXY_PATH &&
    url.searchParams.get(ACUITY_PROXY_FLAG) === '1' &&
    url.searchParams.get('owner') === ACUITY_OWNER &&
    url.searchParams.get('appointmentType') === ACUITY_APPOINTMENT_TYPE;
}

function buildAcuityUrl(url) {
  const acuityUrl = new URL(url.pathname, ACUITY_ORIGIN);
  url.searchParams.forEach((value, key) => {
    if (key !== ACUITY_PROXY_FLAG) acuityUrl.searchParams.set(key, value);
  });
  return acuityUrl;
}
