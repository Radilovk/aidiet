...
8. **admin_emoeat_prompt.txt** - AI анализ на емоционалното хранене
   - Входни данни: 10 избрани отговора от ПРОЕКТИВЕН въпросник (клиентът не отговаря с клинично самоописание, а избира спонтанно от житейски ситуации; + незадължително собствено допълнение) + `{computedProfile}` — детерминистичен профил, изчислен на клиента чрез матрица на тежестите (фактори, HALT, модели, архетипи по косинусова близост, надеждност)
   - Изходни данни: Дълбок индивидуализиран психологически профил с декодирани отговори, архетипи, кръстосани модели, лични прозрения, терапевтични стратегии, HALT профил и спешен протокол
...
Отговори от въпросника: `{answer1}` до `{answer10}` (избран отговор от фиксирани варианти + незадължително собствено допълнение, 10 житейски ситуации)
Изчислен профил: `{computedProfile}` (фактори с интензитет, HALT стойности, открити модели, архетипи с % близост, надеждност — служи като анкор, за да не гадае AI числата)
