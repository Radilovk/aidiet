...
console.log('\n=== 6б. Регресия: хумус/кокосово мляко са храни, не 15g подправки ===');
{
  // Морков + хумус is a legitimate snack; the condiment cap used to crush hummus to
  // 15g and make the calorie target structurally unreachable.
  const meal = { type: 'Хранене 3', description: '• Морков 100g\n• Хумус 50g' };
  applyMealNutritionFromDatabase(meal, { calories: 250, protein: 10, carbs: 30, fats: 10 });
  const hummus = parseMealDescription(meal.description).find(i => i.name === 'Хумус');
  const kcalOk = Math.abs(meal.calories - 250) <= calorieTolerance(250);
  check('хумусът не е орязан до 15g', hummus && hummus.grams > 15, `${hummus?.grams}g`);
  check('закуската достига калорийната цел', kcalOk, `${meal.calories}kcal (цел 250)`);
}
