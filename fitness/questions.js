...
/** Обратно от answers към wizard state (зареждане на запис). */
export function formStateFromAnswers(a) {
  const s = {};
  s.basics = { gender: a.gender || '', age: a.age ?? '', heightCm: a.heightCm ?? '', weightKg: a.weightKg ?? '' };
  ...
  if (a.healthMeds && !healthSel.includes('Приемам медикаменти редовно')) healthSel.push('Приемам медикаменти редовно');
  s.health = { selected: healthSel, inputs: healthInputs };

  if (a.healthFemale?.length) {
    const label = a.healthFemale[0];
    if (label.includes('Бременна')) s.womenContext = { selected: 'Бременна', inputs: { pregnancyTrimester: label.split(':').pop()?.trim() || '' } };
    else if (label.includes('Кърмене')) s.womenContext = { selected: 'Кърмя в момента', inputs: { breastfeedingMonths: label.match(/\d+/)?.[0] || '' } };
    else if (label.includes('Следродилен')) s.womenContext = { selected: 'Скоро след раждане (до 6 месеца)', inputs: { postpartumMonths: label.match(/\d+/)?.[0] || '' } };
    else if (label.includes('Менопауза')) s.womenContext = { selected: 'Менопауза / перименопауза', inputs: {} };
    else s.womenContext = { selected: 'Други хормонални особености', inputs: { womenOther: label } };
  }
  ...
  for (const item of a.limitations || []) {
    ...
    limSel.push(base);
    if (detail && limKeys[base]) limInputs[limKeys[base]] = detail;
  }
  ...
  s.goal = {
    main: a.goal?.main || '', other: a.goal?.other || '',
