...
export const GUIDELINE_CHUNKS = [
  ...
  {
    tags: ['level:среден'],
    text: 'Среден опит (2-5 г): upper/lower или push/pull/legs split, 10-16 серии на група седмично, периодизация на интензитета (тежка/лека седмица или вълнообразна в рамките на седмицата).',
  },
  {
  ...
  },
  {
    ...
    text: 'Следродилен период: постепенно връщане — първо тазово дъно и дълбоки коремни, после базови движения. Внимание за диастаза — без class crunches преди проверка. Интензитетът се вдига едва след 3+ месеца при добро възстановяване.',
  },
  {
...
ПРАВИЛА ЗА ИМЕНУВАНЕ (критично за системата):
- displayName: името на упражнението НА БЪЛГАРСКИ (вижда го клиентът).
- canonicalName: стандартното АНГЛИЙСКО име, точно както се среща в общоприети exercise бази данни (пример: "Barbell Bench Press", "Lat Pulldown", "Dumbbell Romanian Deadlift"). НЕ буквален превод, а реалното каталожно име.
- equipmentHint: на английски, едно от: body weight, dumbbell, barbell, cable, leverage machine, band, kettlebell, stability ball, smith machine, ez barbell.
- bodyPart: на английски, едно от: chest, back, shoulders, upper arms, lower arms, upper legs, lower legs, waist, cardio, neck.
...
ОТГОВОРИ САМО С ВАЛИДЕН JSON без markdown ограждане, точно по тази схема:
{
  ...
  "guidelines": {
    ...
    "adaptation": "кога да олекоти/затежни тренировката според усещането"
  }
}`;
...
export function normalizePlan(plan) {
  ...
  const normDays = days.slice(0, 7).map((d, i) => ({
    ...
    exercises: (Array.isArray(d.exercises) ? d.exercises : []).slice(0, 12).map((ex) => ({
      displayName: String(ex.displayName || ex.canonicalName || 'Упражнение'),
...
- Ако клиентът иска промяна на плана, обясни как да ползва бутоните за замяна/олекотяване/затежняване в приложението, или дай конкретна устна корекция.`;
