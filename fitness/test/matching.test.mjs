...
test('selectGuidelineLayers: архитектурни chunks са отделен слой', () => {
  const layers = selectGuidelineLayers(new Set(['goal:отслабване']), {
    chunks: [
...
test('buildAdminPlanUserPrompt: женски профил + идентичност най-отгоре', () => {
  ...
  }, { individual: ['Насока за жена'], architecture: [] }, 'Принцип А');
...
test('buildPlanUserPrompt: individual преди architecture', () => {
  const prompt = buildPlanUserPrompt('Профил', { individual: ['Насока 1'], architecture: ['Арх 1'] }, 'Принцип А');
  assert.ok(prompt.includes('ИНДИВИДУАЛЕН ПРОФИЛ'));
  assert.ok(prompt.includes('Принцип А'));
  assert.ok(prompt.includes('Насока 1'));
  assert.ok(prompt.includes('Арх 1'));
  assert.ok(prompt.indexOf('ИНДИВИДУАЛЕН') < prompt.indexOf('АРХИТЕКТУРНА'));
});
...
test('buildAdminPlanUserPrompt: включва профил, схема и указания от треньора', () => {
  ...
  }, { individual: [], architecture: ['Универсално правило'] }, 'Принцип А');
...
test('selectGuidelineLayers: админ chunks не се режат от hardcoded fallback', () => {
  const layers = selectGuidelineLayers(new Set(['goal:отслабване', 'gender:жена', 'level:начинаещ']), {
...
test('production admin config: questionnaire tags влизат в prompt', async () => {
  ...
  const layers = selectGuidelineLayers(tags, config);
  const joined = layers.individual.join('\n');
  assert.ok(joined.includes('ПОЛ'), 'production пол chunk');
  assert.ok(joined.includes('глутеус') || joined.includes('глут'), 'production женски акцент');
  assert.ok(joined.includes('ОБОРУДВАНЕ') || joined.includes('В13'), 'production equipment chunk');
...
test('QUESTIONS: 16 стъпки (жена вижда womenContext + womenImplants)', () => {
  assert.equal(QUESTIONS.length, 16);
  assert.ok(QUESTIONS.some((q) => q.id === 'womenContext'));
  assert.ok(QUESTIONS.some((q) => q.id === 'womenImplants'));
  assert.equal(activeQuestions({ basics: { gender: 'Мъж' } }).length, 14);
  assert.equal(activeQuestions({ basics: { gender: 'Жена' } }).length, 16);
});

test('activeQuestions: женският блок е отделна стъпка', () => {
