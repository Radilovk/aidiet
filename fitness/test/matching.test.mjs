...
test('constraintsFromAnswers: зони↓ са първи и заместват женския default', () => {
  const c = constraintsFromAnswers({
    gender: 'Жена',
    goal: { main: 'Отслабване', zones: 'бедра, корем' },
    equipment: [], limitations: [], health: [], healthFemale: [],
  });
  assert.equal(c.priorities[0], 'Зони↓: бедра, корем');
  assert.ok(!c.priorities.some((p) => /дупе>бедра/i.test(p)));
});
...
test('buildAdminPlanUserPrompt: зони в constraints', () => {
  const prompt = buildAdminPlanUserPrompt({
  ...
  assert.ok(prompt.includes('Зони↓: бедра, корем'));
