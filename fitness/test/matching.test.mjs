...
test('selectGuidelines: извлича само релевантните порции (individual слой)', () => {
  ...
  assert.ok(/сърдечно-съдов/i.test(joined), 'очаква се насока за хипертония');
  ...
  assert.ok(!joined.includes('Хипертрофия'), 'goal/level насоки са в program_spec, не в RAG');
});
...
test('selectGuidelines: бременност и начинаещ', () => {
  ...
  const joined = individual.join('\n');
  assert.ok(/бременност/i.test(joined));
...
test('selectGuidelines: слива админ chunks и ограничава обема', () => {
  ...
  assert.ok(!joined.includes('комбинирай съпротивителен тренинг'), 'hardcoded goal chunks са премахнати — само админ');
...
test('buildAnswers: всички свободни текстови полета се предават', () => {
  const state = {
    basics: { gender: 'Мъж', age: '30', heightCm: '180', weightKg: '80' },
    health: {
      selected: ['Друго', 'Приемам медикаменти редовно'],
      inputs: { healthOther: 'астма хронична', healthMeds: 'вентолин' },
    },
    womenContext: { selected: 'Няма специфични състояния', inputs: {} },
    womenImplants: { selected: 'Нямам гръдни импланти', inputs: {} },
    limitations: { selected: ['Друго'], inputs: { limitOther: 'болка в китката' } },
    weightChange: { selected: 'Да, свалих килограми', inputs: { lossKg: '5', lossReason: 'стрес на работа' } },
    sleep: { selected: 'Добро, събуждам се отпочинал/а' },
    stress: 4,
    dailyActivity: { selected: 'Заседнала работа, минимално движение' },
    sportActivity: { selected: 'Тренирам системно', inputs: { sportCurrent: 'плуване 2 пъти седмично' } },
    experience: { selected: 'Начинаещ–среден (6 месеца – 2 години)' },
    nutrition: { type: 'Специфична диета', custom: 'безглутенова', mealsPerDay: '3' },
    goal: { main: 'Друго', other: 'подготовка за маратон', zones: '', timeframe: 'Конкретна дата', deadline: '15 май 2027' },
    equipment: { selected: ['Друго'], inputs: { equipmentOther: 'trx в къщи' } },
    preferences: { types: ['Силов тренинг'], avoid: 'бърпи с клек', freq: '3–4', duration: '45–60 мин', timeOfDay: 'Сутрин' },
    extraInfo: 'работя нощни смени',
  };
  const answers = buildAnswers(state);

  assert.ok(answers.health.includes('астма хронична'));
  assert.equal(answers.healthOther, 'астма хронична');
  assert.equal(answers.healthMeds, 'вентолин');
  assert.deepEqual(answers.limitations, ['Друго: болка в китката']);
  assert.equal(answers.weightChange.reason, 'стрес на работа');
  assert.equal(answers.sportActivity.current, 'плуване 2 пъти седмично');
  assert.equal(answers.nutrition.custom, 'безглутенова');
  assert.equal(answers.goal.other, 'подготовка за маратон');
  assert.equal(answers.goal.deadline, '15 май 2027');
  assert.equal(answers.equipmentOther, 'trx в къщи');
  assert.equal(answers.preferences.avoid, 'бърпи с клек');
  assert.equal(answers.extraInfo, 'работя нощни смени');
});
