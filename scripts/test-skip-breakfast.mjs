...
const chunkSrc = readFileSync('step3-chunk.js', 'utf8');
const strategyPrompt = readFileSync('KV/prompts/admin_strategy_prompt.txt', 'utf8');
const step3Task = chunkSrc + '\n' + mealPrompt;

assert(step3Task.includes('meals[].type MUST match'), 'Step 3 slot contract in English');
assert(!mealPrompt.includes('"type": "Хранене 1"'), 'no H1 anchoring example');
assert(step3Task.includes('name — dish title'), 'name vs description split');
...
assert(step3Task.includes('catalog group names'), 'prefer catalog generics when precision not required');
