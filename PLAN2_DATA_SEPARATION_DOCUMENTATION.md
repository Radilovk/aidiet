...
## ВАЖНО УТОЧНЕНИЕ - Какво е Сектор 3?

**Сектор 3 НЕ е "всички данни които се предават към следващата стъпка".**

**Сектор 3 е "данни от АНАЛИЗА/ИЗХОДА на текущата стъпка, които са полезни за следващата стъпка".**

### Пълната формула за входа на всяка стъпка:

```
Вход на Стъпка N = 
  userData (клиентски данни от въпросника) +
  Сектор 3 данни от Стъпка N-1 (анализ от предишната стъпка) +
  Допълнителни елементи (whitelists, blacklists, previousDays и др.)
```

**Примери:**
- **Стъпка 2 получава:** userData + Sector 3 от Стъпка 1
- **Стъпка 3 получава:** userData + Sector 3 от Стъпка 1 + Sector 3 от Стъпка 2 + whitelists + blacklists
- **Стъпка 4 получава:** userData + избрани данни от Стъпки 1-3 + whitelists + blacklists
...
3. **СЕКТОР 3: ДАННИ ЗА СЛЕДВАЩА СТЪПКА**
   - Цел: Контекст от анализа на текущата стъпка за следващата стъпка
   - Бележка: Данните се предават в КОМПАКТЕН формат за ефективност
   - **ВАЖНО:** Сектор 3 съдържа САМО данните от анализа на ТЕКУЩАТА стъпка
   - **ВАЖНО:** Следващата стъпка получава също userData и допълнителни елементи
