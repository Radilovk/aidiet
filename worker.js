...
async function generateCorrectionPrompt(plan, validationErrors, userData, env) {
  ...
  const additionalNotesSection = userData.additionalNotes
  ...
  let prompt = replacePromptVariables(customPrompt, {
      ...
      dietDislike: userData.dietDislike || 'няма',
      dietLove: userData.dietLove || 'няма',
...
async function getChatPrompts(env) {
  ...
  // Prompts are loaded exclusively from KV storage; no inline fallbacks.
  // If KV prompts are not configured, prompts will be undefined and chat will fail gracefully.
  const prompts = {};
