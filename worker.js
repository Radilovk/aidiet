...
// Note: This is the OUTPUT token limit. Input prompt optimization is crucial for staying within API limits.
...
function generateAnalysisPrompt(data) {
  return `Ти си експерт диетолог, ендокринолог и психолог. Направи ХОЛИСТИЧЕН АНАЛИЗ на клиента:

КЛИЕНТ: ${data.name}, ${data.gender}, ${data.age}год., ${data.height}см, ${data.weight}кг
ЦЕЛ: ${data.goal}${data.lossKg ? ` (${data.lossKg}кг)` : ''}

ЗДРАВЕ: Сън ${data.sleepHours}ч${data.sleepInterrupt !== 'Не' ? ' (прекъсван)' : ''}, ${data.chronotype}, Стрес ${data.stressLevel}, Активност ${data.sportActivity}, Вода ${data.waterIntake}

НАВИЦИ: Преяждане ${data.overeatingFrequency}, ${JSON.stringify(data.eatingHabits || [])}${data.foodCravings && data.foodCravings.length > 0 ? `, Желания: ${JSON.stringify(data.foodCravings)}` : ''}${data.foodTriggers && data.foodTriggers.length > 0 ? `, Тригери: ${JSON.stringify(data.foodTriggers)}` : ''}

${data.medicalConditions && data.medicalConditions.length > 0 ? `МЕДИЦИНСКИ: ${JSON.stringify(data.medicalConditions)}${data.medications === 'Да' ? `, Лекарства: ${data.medicationsDetails}` : ''}` : ''}

ИЗИСКВАНИЯ:
1. Изчисли ИНДИВИДУАЛНО BMR, TDEE, калории и макроси за ${data.name}
2. Анализирай корелации: сън-стрес-хранене, медицински състояния-нужди
3. Идентифицирай 3-6 КЛЮЧОВИ ПРОБЛЕМА (САМО Borderline/Risky/Critical severity, НИКОГА Normal)
4. Шанс за успех: -100 до 100 (отрицателен при противоречащи фактори)

JSON ФОРМАТ:
{
  "bmr": "число + кратко обяснение",
  "tdee": "число + кратко обяснение",
  "recommendedCalories": "число според цел ${data.goal}",
  "macroRatios": {"protein": "X% защото...", "carbs": "X% защото...", "fats": "X% защото..."},
  "metabolicProfile": "КРАТКО описание на метаболизма на ${data.name}",
  "healthRisks": ["риск 1", "риск 2"],
  "nutritionalNeeds": ["нужда 1", "нужда 2"],
  "psychologicalProfile": "Емоционално хранене, стрес, поведение",
  "successChance": число,
  "successChanceReasoning": "защо този шанс",
  "keyProblems": [{"title": "2-4 думи", "description": "до 3 изречения", "severity": "Borderline/Risky/Critical", "severityValue": 0-100, "category": "Sleep/Nutrition/Hydration/Stress/Activity/Medical", "impact": "кратко"}]
}`;
}
...
АНАЛИЗ: ${analysisSummary}
ПРЕДПОЧИТАНИЯ: ${JSON.stringify(data.dietPreference || [])}${data.dietDislike ? `, Избягва: ${data.dietDislike}` : ''}${data.dietLove ? `, Обича: ${data.dietLove}` : ''}
${data.medicalConditions && data.medicalConditions.length > 0 ? `МЕДИЦИНСКИ: ${JSON.stringify(data.medicalConditions)}` : ''}

ИЗИСКВАНИЯ:
1. Определи МОДИФИКАТОР (диетичен профил): Балансирано, Кето, Палео, Веган, Средиземноморско, Нисковъглехидратно, Щадящ стомах, или друг
2. Създай седмична схема (може да включва интермитентно гладуване 16:8, циклично, или стандартна)
3. ИНДИВИДУАЛНИ добавки за ${data.name} (${data.age}год., ${data.gender}, ${data.goal}, активност ${data.sportActivity}, сън ${data.sleepHours}ч)

JSON ФОРМАТ:
{
  "dietaryModifier": "термин (Балансирано/Кето/Веган/и др.)",
  "modifierReasoning": "защо избран за ${data.name}",
  "planJustification": "КРАТКО (до 200 символа) защо планът е индивидуален",
  "dietType": "тип диета",
  "weeklyMealPattern": "седмична схема (напр. 16:8, 5:2, или стандартна)",
  "mealTiming": {"pattern": "модел", "fastingWindows": "периоди", "flexibility": "гъвкавост"},
  ...
  "supplementRecommendations": [
    "ИНДИВИДУАЛНА добавка 1 - име, дозировка, защо нужна за ${data.name}",
    "ИНДИВИДУАЛНА добавка 2 - име, дозировка, защо нужна за ${data.name}",
    "ИНДИВИДУАЛНА добавка 3 - име, дозировка, защо нужна за ${data.name}"
  ],
  "hydrationStrategy": "прием на течности",
  "psychologicalSupport": ["съвет 1 за ${data.name}", "съвет 2 за ${data.name}", "съвет 3 за ${data.name}"]
}`;
}
...
КЛИЕНТ: ${data.name}, ${data.age} год., ${data.gender}, Цел: ${data.goal}
МЕТАБОЛИЗЪМ: ${analysisSummary}
СТРАТЕГИЯ: ${dietaryModifier} - ${strategySummary}
КАЛОРИИ: ${recommendedCalories} kcal/ден (средно за седмицата)
ИЗБЯГВАЙ: ${foodsToAvoid}
ВКЛЮЧВАЙ: ${foodsToInclude}
${modificationsSection}
${data.medicalConditions && data.medicalConditions.length > 0 ? `ЗДРАВОСЛОВНИ ОГРАНИЧЕНИЯ: ${data.medicalConditions.join(', ')}` : ''}

ПРАВИЛА ЗА ГЕНЕРИРАНЕ:
1. ФИЛТРИРАЙ храни според диетата "${dietaryModifier}":
   - Веган: само растителни продукти
   - Кето/Нисковъглехидратно: минимум въглехидрати, повече протеини и мазнини
   - Без глутен: без пшеница, ръж, ечемик
   - Палео: без зърнени, бобови, млечни
   - Щадящ: готвени зеленчуци, без сурови влакнини

2. СТРУКТУРА НА ЯСТИЯТА:
   - Закуска: лека (яйца, кисело мляко, овес) или сандвич
   - Обяд/Вечеря: Протеин + Въглехидрати + Зеленчуци ИЛИ Сложно ястие + Салата
   - Използвай БАЛАНСИРАНИ и ТРАДИЦИОННИ български/средиземноморски ястия

3. ИЗИСКВАНИЯ:
   - Всички 7 дни (day1-day7) с 2-4 хранения според стратегията${data.eatingHabits && data.eatingHabits.includes('Не закусвам') ? '\n   - БЕЗ закуска (клиентът не закусва)' : ''}
   - РАЗЛИЧНИ ястия всеки ден (без повторение)
   - Прости, достъпни продукти в България
   - Точни калории и макроси за всяко ястие
   - Общо ${recommendedCalories} kcal/ден (средно)

4. ЗАБРАНЕНО:
   - Странни комбинации (чийзкейк със салата)
   - Екзотични продукти
   - Повторение на храни
   - Общи съвети вместо конкретни храни в recommendations/forbidden
...
Създай пълния 7-дневен план (day1 до day7) с балансирани, реалистични и индивидуални ястия.`;
}
