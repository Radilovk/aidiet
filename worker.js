...
- Протеин: {protein}% ({proteinGramsPerKg}g/kg за {gender})
  * {gender === 'Мъж' ? 'Мъже имат по-високи нужди' : 'Жени имат по-ниски нужди'} от протеин
...
**СЪН ↔ СТРЕС ↔ ХРАНЕНЕ**: Как {sleepHours}ч сън (прекъсван: {sleepInterrupt}) + стрес ({stressLevel}) влияят на:
   - Хормони (кортизол, грелин, лептин)
   - Хранителни желания: {JSON.stringify foodCravings}
   - Прекомерно хранене: {overeatingFrequency}

**ПСИХОЛОГИЧЕСКИ ПРОФИЛ**: Анализирай връзката емоции ↔ хранене:
   - Тригери: {JSON.stringify foodTriggers}
   - Компенсации: {JSON.stringify compensationMethods}
...
Идентифицирай уникален метаболитен профил базиран на:
   ...
   1. ПСИХОЛОГИЧЕСКИ ПРОФИЛ ↔ ХРОНОТИП:
      - Емоционално хранене: {JSON.stringify foodTriggers}
      - Как хронотипът влияе на самоконтрол през деня
      - Кога {name} е най-податлив на тригери
      - Времето на ден с най-добра дисциплина
   
   2. ЗДРАВЕН СТАТУС ↔ ХРОНОТИП:
      - Медицински състояния: {JSON.stringify medicalConditions}
      - Как хронотипът влияе на симптомите
      - Оптимално време за хранене според здравето
      - Периоди за избягване на тежка храна
   ...
   - История: {dietHistory === 'Да' ? `{dietType} → {dietResult}` : 'няма предишни диети'}
   - ВАЖНО: Неуспешни диети в миналото обикновено означават намален метаболизъм

**МЕДИЦИНСКИ ФАКТОРИ И ЛЕКАРСТВЕНИ ВЗАИМОДЕЙСТВИЯ (Issue #3 - ФАЗА 2)**:
КРИТИЧНО ВАЖНО - AI МОДЕЛ ЗАДЪЛЖИТЕЛНО ТРЯБВА ДА АНАЛИЗИРА ЛЕКАРСТВА:
   - Състояния: {JSON.stringify medicalConditions}
   - Лекарства: {medications === 'Да' ? medicationsDetails : 'не приема'}
   
   {if medications === 'Да' && medicationsDetails: `
   ⚠️ ЗАДЪЛЖИТЕЛНА ПРОВЕРКА ЗА ЛЕКАРСТВЕНИ ВЗАИМОДЕЙСТВИЯ:
...
3. **ШАНС ЗА УСПЕХ (Issue #20 - ФАЗА 2)**: 
   ...
   СПЕЦИФИЧНИ КОРЕКЦИИ:
   - Неуспешна диета в миналото: {dietHistory === 'Да' ? `{dietType} → {dietResult}` : 'няма'} 
     {if dietHistory === 'Да' && dietResult includes 'неуспешн': '→ намали с 15-25 точки'}
   - Метаболитна адаптация: {dietHistory === 'Да' ? 'вероятна → намали още 10-15 точки' : 'не се очаква'}
   - Психологическа бариера: анализирай {JSON.stringify foodTriggers}
   - Медицински бариери: {JSON.stringify medicalConditions}
...
{if weeklyBlueprint: `- Седмична структура: {weeklyBlueprint.dailyMealCount} хранения/ден{weeklyBlueprint.skipBreakfast ? ', БЕЗ закуска' : ''}`}
...
ПРЕДПОЧИТАНИЯ:
- Диетични предпочитания: {JSON.stringify dietPreference}
{if dietPreference_other: `  (Друго: {dietPreference_other})`}
- Не обича/непоносимост: {dietDislike || 'Няма'}
- Любими храни: {dietLove || 'Няма'}
...
5. Вземи предвид медицински състояния, лекарства и възможни взаимодействия
6. КРИТИЧНО - ПРОВЕРКА ЗА ВЗАИМОДЕЙСТВИЯ:
   - Ако клиентът приема лекарства: {medications === 'Да' ? medicationsDetails : 'не приема'}, провери:
     * Витамин К + антикоагуланти (варфарин) = противопоказано
     * Калций/Магнезий + антибиотици = намалено усвояване
     * Желязо + антациди = блокирано усвояване
     * Витамин D + кортикостероиди = необходима по-висока доза
   - Ако има медицински състояния: {JSON.stringify medicalConditions}, съобрази:
     * Диабет: Хром, Витамин D, Омега-3 (контрол на кръвна захар)
     * Хипертония: Магнезий, Калий, CoQ10 (понижаване на налягане)
     * Щитовидна жлеза: Селен, Йод (само ако е дефицит!), Цинк
     * Анемия: Желязо (хемово за по-добро усвояване), Витамин C (подпомага усвояването), B12
...
Върни JSON със стратегия (БЕЗ универсални препоръки):
{
  ...
  "supplementRecommendations": [
    "! ИНДИВИДУАЛНА добавка 1 за {name} - конкретна добавка с дозировка и обосновка защо Е НУЖНА за този клиент (БАЗИРАНА на: възраст {age} год., пол {gender}, цел {goal}, медицински състояния {medicalConditions || 'няма'})",
...
- Вземи предвид уникалната комбинация от: {age} год. {gender}, {goal}, {medicalConditions || 'няма мед. състояния'}, {sportActivity}, стрес: {stressLevel}`,
...
Избягвай: {dietDislike || 'няма'}, {foodsToAvoid}
Включвай: {dietLove || 'няма'}, {foodsToInclude}{previousDaysContext}
...
Категории: [PRO]=Белтък, [ENG]=Енергия/въглехидрати, [VOL]=Зеленчуци/фибри, [FAT]=Мазнини, [CMPX]=Сложни ястия
Шаблони: A) РАЗДЕЛЕНА ЧИНИЯ=[PRO]+[ENG]+[VOL], B) СМЕСЕНО=[PRO]+[ENG]+[VOL] микс, C) ЛЕКО/САНДВИЧ, D) ЕДИНЕН БЛОК=[CMPX]+[VOL]
Филтриране според "{dietaryModifier}": Веган=без животински [PRO]; Кето=минимум [ENG]; Без глутен=[ENG] само ориз/картофи/киноа/елда; Палео=без зърнени/бобови/млечни{if eatingHabits && eatingHabits.includes('Не закусвам'): `\nЗАКУСКА: Клиентът НЕ ЗАКУСВА - без закуска или само напитка ако критично`}
