...
6. Ред: Закуска → Обяд → (Следобедна закуска) → Вечеря → (Късна закуска само ако: >4ч между вечеря и сън + обосновано: диабет, интензивни тренировки)
   - ВАЖНО: Закуска (сутрешно хранене), Следобедна закуска (междинно хранене следобед), Късна закуска (късно хранене преди сън) - ТРИ РАЗЛИЧНИ ТИПА!
   - Късна закуска САМО с low GI: кисело мляко, ядки, ягоди/боровинки, авокадо, семена (макс ${MAX_LATE_SNACK_CALORIES} kcal)
7. Разнообразие: Различни ястия от предишните дни${data.eatingHabits && data.eatingHabits.includes('Не закусвам') ? '\n8. ВАЖНО ПРАВИЛО ЗА ЗАКУСКА: Клиентът НЕ ЗАКУСВА традиционно!\n   - ЗАБРАНЕНО: Твърда храна като закуска (хляб, яйца, каша и др.)\n   - РАЗРЕШЕНО: САМО напитки като компромис - айран (200-250ml), зелен чай, черен чай, вода с лимон, билков чай\n   - Калории от напитката: 0-50 kcal максимум\n   - Обяснение: "Сутрешна напитка вместо закуска според навиците на клиента"' : ''}
...
НАПОМНЯНЕ ЗА ТИПОВЕ ХРАНЕНИЯ:
- "Закуска" = сутрешно хранене (breakfast)
- "Обяд" = обедно хранене (lunch)
- "Следобедна закуска" = междинно следобедно хранене (afternoon snack)
- "Вечеря" = вечерно хранене (dinner)
- "Късна закуска" = късно хранене преди сън (late-night snack)
...
ТИПОВЕ ХРАНЕНИЯ (ВАЖНО - 5 различни типа):
- "Закуска" = сутрешно хранене (breakfast)
- "Обяд" = обедно хранене (lunch)
- "Следобедна закуска" = междинно следобедно хранене (afternoon snack)
- "Вечеря" = вечерно хранене (dinner)
- "Късна закуска" = късно хранене преди сън (late snack)
...
1. ПОЗВОЛЕНИ ТИПОВЕ ХРАНЕНИЯ - 5 РАЗЛИЧНИ типа (в хронологичен ред):
   - "Закуска" (сутрин) - СУТРЕШНО хранене
   - "Обяд" (обед) - ОБЕДНО хранене
   - "Следобедна закуска" (опционално, след обяд) - МЕЖДИННО СЛЕДОБЕДНО хранене
   - "Вечеря" (вечер) - ВЕЧЕРНО хранене
   - "Късна закуска" (опционално, след вечеря - С ОБОСНОВКА!) - КЪСНО хранене преди сън
...
2. СПЕЦИАЛНИ СЛУЧАИ:
   a) Ако клиентът НЕ ЗАКУСВА (проверка: "Не закусвам" в eatingHabits):
      - Закуската като твърда храна ОТПАДА напълно
      - ЗАДЪЛЖИТЕЛНО препоръчай напитка като компромис:
        * Айран (200-250ml, ~50-80 kcal) - пробиотици и лек протеин
        * Зелен чай - антиоксиданти и метаболизъм
        * Черен чай или кафе - бодрост
        * Вода с лимон - хидратация и витамин C
        * Билков чай - според нуждите (мента, лайка и др.)
      - Обясни в breakfastStrategy защо напитката е подходяща
      - Обясни в mealTiming как се вписва в дневния ритъм
      - В weeklyBlueprint задай skipBreakfast: true
...
{eatingHabits} - Ако клиентът не закусва: ЗАКУСКА ЗАБРАНЕНА! Позволени САМО напитки: айран (200-250ml), зелен/черен/билков чай, вода с лимон. Калории: 0-50 kcal макс.
...
1. ПОЗВОЛЕНИ ТИПОВЕ ХРАНЕНИЯ - 5 РАЗЛИЧНИ типа (в хронологичен ред):
   - "Закуска" (сутрин) - СУТРЕШНО хранене, САМО като първо хранене на деня
   - "Обяд" (обед) - ОБЕДНО хранене, САМО след закуската или като първо хранене (ако няма закуска)
   - "Следобедна закуска" (опционално, между обяд и вечеря) - МЕЖДИННО СЛЕДОБЕДНО хранене
   - "Вечеря" (вечер) - ВЕЧЕРНО хранене, обикновено последно хранене
   - "Късна закуска" (опционално, САМО след вечеря, специални случаи) - КЪСНО хранене преди сън
