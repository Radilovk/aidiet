...
Основни физически параметри (за референция при изчисления):
...
ВАЖНО: Използвай Mifflin-St Jeor формула като ОТПРАВНА ТОЧКА, но КОРИГИРАЙ базирано на:
- Качество на сън и стрес (влияят на метаболизма)
...
═══ КАК ДА ИЗЧИСЛИШ КАЛОРИИТЕ ХОЛИСТИЧНО ═══

1. BMR ИЗЧИСЛЕНИЕ:
   Формула Mifflin-St Jeor: 10×тегло(кг) + 6.25×височина(см) - 5×възраст(год) + 5 (мъже) / -161 (жени)
   
   КОРЕКЦИИ (примери):
   - Лош сън (< 6ч, прекъсван): -5% до -10% (кортизол ↑, метаболизъм ↓)
   - Висок стрес: -5% до -8% (хроничен стрес забавя метаболизма)
   - Неуспешни диети в миналото: -10% до -15% (метаболитна адаптация, body "се пази")
   - Хипотиреоидизъм: -10% до -20% (намален базов метаболизъм)
   - Добър сън + нисък стрес: +0% до +5% (оптимален метаболизъм)

2. TDEE ИЗЧИСЛЕНИЕ (BMR × Activity Factor):
   Стандартни множители:
   - Никаква активност: 1.2
   - Ниска (1-2 дни): 1.375
   - Средна (2-4 дни): 1.55
   ...
   КОРЕКЦИИ:
   - Ако дневната активност е също висока → увеличи множителя с 0.05-0.1
   - Ако стресът е висок → намали множителя с 0.05 (намалена физическа ефективност)
   - Ако съня е лош → намали множителя с 0.05 (по-малко енергия за активност)

3. ПРЕПОРЪЧАНИ КАЛОРИИ (базирани на цел):
   Стандартни правила:
   ...
   КОРЕКЦИИ:
   - Агресивно отслабване (${data.lossKg ? `цел: ${data.lossKg}кг` : 'няма'}): TDEE × 0.75-0.80 (20-25% дефицит) - САМО ако няма история на диети!
   - История на йо-йо диети → ПО-МАЛЪК дефицит (TDEE × 0.90, 10% дефицит) за устойчивост
   - Висок стрес + емоционално хранене → ПО-МАЛЪК дефицит (TDEE × 0.90) - твърде агресивно = стрес = провал
   - Медицински състояния (диабет, СПКЯ) → консервативен подход, по-малък дефицит

ПРИМЕРИ:
- Жена, 35г, 70кг, 165см, средна активност, ДОБЪР сън, нисък стрес, БЕЗ история на диети:
  BMR = 1395 (без корекция), TDEE = 1395 × 1.55 = 2162, Отслабване = 2162 × 0.85 = 1838 kcal

- Жена, 35г, 70кг, 165см, средна активност, ЛОШ сън (5ч), висок стрес, 3 неуспешни диети:
  BMR = 1395 × 0.85 = 1186 (15% корекция за стрес+сън+история)
  TDEE = 1186 × 1.50 = 1779 (намален множител заради стрес/сън)
  Отслабване = 1779 × 0.90 = 1601 kcal (по-малък дефицит заради история)

═══ ТВОЯТА ЗАДАЧА ═══
1. ИЗЧИСЛИ BMR, TDEE и препоръчани калории ХОЛИСТИЧНО - не просто формули!
   - Започни с Mifflin-St Jeor, но КОРИГИРАЙ според всички корелати
   - Обясни ЗАЩО си избрал точно тези стойности
   - Покажи математиката в reasoning полетата
