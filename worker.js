...
function getDefaultPromptTemplates() {
  // NOTE: This function returns template strings with {placeholders} for display in admin panel.
...
**МЕТАБОЛИТНИ ОСОБЕНОСТИ И ХРОНОТИП**: 
Идентифицирай уникален метаболитен профил базиран на:
...
**МЕДИЦИНСКИ ФАКТОРИ И ЛЕКАРСТВЕНИ ВЗАИМОДЕЙСТВИЯ**:
КРИТИЧНО ВАЖНО - AI МОДЕЛ ЗАДЪЛЖИТЕЛНО ТРЯБВА ДА АНАЛИЗИРА ЛЕКАРСТВА:
   - Състояния: {medicalConditions}
...
JSON структура с всички изисквани полета за холистичен анализ.
...
Върни JSON със стратегия (БЕЗ универсални препоръки) включително dietaryModifier, dietType, weeklyMealPattern, mealTiming, keyPrinciples, foodsToInclude, foodsToAvoid, supplementRecommendations, hydrationStrategy, psychologicalSupport.`,
...
Включва пълната логика на ADLE системата:
- Hard bans (лук, пуешко, изкуствени подсладители, мед, захар и др.)
- Whitelist протеини, зеленчуци, енергийни източници
- R1-R12 правила за композиция на ястията
- Корелационна адаптация според стрес, хронотип и сън

=== КРИТИЧНИ ИЗИСКВАНИЯ ===
1. ЗАДЪЛЖИТЕЛНИ МАКРОСИ за всяко ястие
2. ПРЕЦИЗНИ КАЛОРИИ (protein×4 + carbs×4 + fats×9)
3. ЦЕЛЕВА ДНЕВНА СУМА: около {recommendedCalories} kcal
4. БРОЙ ХРАНЕНИЯ: 1-6 според стратегията
5. РАЗНООБРАЗИЕ между дните
6. Реалистични български/средиземноморски ястия
...
JSON ФОРМАТ (върни САМО дните {startDay}-{endDay}):
{
  "day{startDay}": {
    "meals": [
      {"type": "Закуска", "name": "име", "weight": "Xg", "description": "описание", "benefits": "ползи", "calories": X, "macros": {"protein": X, "carbs": X, "fats": X, "fiber": X}}
    ],
    "dailyTotals": {"calories": X, "protein": X, "carbs": X, "fats": X}
  }
}`,
