...
ФИЛТРИ по модификатор: Веган=без животински ПРО; Кето=мин ЕНГ; Без глутен=ЕНГ само ориз/картофи; Палео=без зърнени/бобови/млечни
ЗАБРАНИ(0%): ${ADLE_V8_HARD_BANS.slice(0, 8).join(', ')}; грах+риба забранени
РЯДКО(≤2/седм): ${ADLE_V8_RARE_ITEMS.join(', ')}
ПРАВИЛА: R1:ПРО=1. R2:ВОЛ=1-2(само 1 форма). R3:ЕНГ=0-1. R4:Млечни≤1. R5:МАЗ=0-1. R6:Сирене→без масло. R7:Бекон→МАЗ=0. R8:Бобови-основно→ЕНГ=0. R9:Хляб само ако ЕНГ=0. R10:Грах-добавка→ЕНГ=0. R11:Template C само закуски. R12:Само whitelist (изключения само с Reason:)
СПЕЦИАЛНИ: Зеленчуци=Салата ИЛИ Пресни(не двете); Маслини≠МАЗ; Царевица≠ЕНГ; Template C≠основни хранения`;
...
function generateMealPlanChunkPrompt(data, analysis, strategy, bmr, recommendedCalories, startDay, endDay, previousDays) {
  ...
  // Build modifications section (compressed)
  ...
  // Build previous days context (compressed)
  let previousDaysContext = '';
  if (previousDays.length > 0) {
    const prevMeals = previousDays.map(d => {
      const mealNames = d.meals.map(m => m.name).join(', ');
      return `Д${d.day}: ${mealNames}`;
    }).join('; ');
    previousDaysContext = `\nВЕЧЕ ГЕНЕРИРАНИ: ${prevMeals}\nИЗБЯГВАЙ повтаряне!`;
  }
  
  // COMPRESSED PROMPT - removed verbosity, kept essential logic
  return `${ARCHPROMPT_COMPRESSED}

ЗАДАЧА: Генерирай дни ${startDay}-${endDay} за ${data.name}
КЛИЕНТ: Цел=${data.goal}, Кал=${recommendedCalories}/ден, BMR=${bmr}, Мод="${dietaryModifier}"${modificationsSection}
...
7. РАЗНООБРАЗИЕ: Всеки ден различен

JSON (върни САМО дни ${startDay}-${endDay}):
{
  "day${startDay}": {
    "meals": [{"type":"Закуска","name":"...","weight":"Xg","description":"...","benefits":"...","calories":X,"macros":{"protein":X,"carbs":X,"fats":X,"fiber":X}}, ...],
    "dailyTotals": {"calories":X,"protein":X,"carbs":X,"fats":X}
  }${daysInChunk > 1 ? `,\n  "day${startDay + 1}": {...}` : ''}
}

Генерирай дни ${startDay}-${endDay} с балансирани ястия в правилен ред. ЗАДЪЛЖИТЕЛНО dailyTotals!`;
}
/**
