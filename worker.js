...
Създай прост, практичен план.`;
  ...
  // Get hacks from KV storage based on user's goal (NOT AI-generated)
  const hacks = await getGoalHacks(data.goal, env);
...
/**
 * Get hardcoded hacks for a specific goal from KV storage
 * Hacks are managed via admin panel, NOT generated by AI
...
async function getGoalHacks(goal, env) {
  try {
    if (env && env.page_content) {
      ...
      const goalHacksData = await env.page_content.get('goal_hacks');
      if (goalHacksData) {
        ...
        // If goal not found in KV, use "Друго" as fallback
        if (allHacks["Друго"]) {
          return allHacks["Друго"];
        }
      }
    }
  } catch (error) {
...
7. Разнообразие: Различни ястия от предишните дни${data.eatingHabits && data.eatingHabits.includes('Не закусвам') ? '\n8. ВАЖНО: Клиентът НЕ ЗАКУСВА - без "Закуска", без "Следобедна закуска" и без "Късна закуска"! Само Обяд и Вечеря (и евентуално едно друго основно хранене).' : ''}${sweetsCravingRule}${buildFreeMealInstruction(strategy, startDay, endDay)}
...
Ти си експертен клиничен диетолог, ендокринолог, психолог и специалист по поведенческа промяна. Тази стъпка поема аналитичната отговорност за препоръки за хранителни добавки, психологическа подкрепа и конкретни позволени/забранени храни.
  ...
  try {
    ...
    if (earliestErrorStep === 'step1_analysis' || earliestErrorStep === 'step2_strategy' || earliestErrorStep === 'step3_mealplan') {
      ...
      if (!summaryData || summaryData.error) {
        ...
        // Get hacks from KV storage based on user's goal (NOT AI-generated)
        const hacksForGoal = await getGoalHacks(data.goal, env);
...
/**
 * Get all goal hacks from KV storage
 */
async function handleGetGoalHacks(request, env) {
  try {
    ...
    const raw = await env.page_content.get('goal_hacks');
    const hacks = raw ? JSON.parse(raw) : getDefaultGoalHacks();
    return jsonResponse({ success: true, hacks });
  } catch (error) {
...
/**
 * Set all goal hacks (replaces existing)
 */
async function handleSetGoalHacks(request, env) {
  try {
    ...
    if (!hacks || typeof hacks !== 'object') {
      return jsonResponse({ error: 'Invalid hacks format' }, 400);
    }
...
async function handleAddGoalHack(request, env) {
  try {
    ...
    const hack = (data.hack || '').trim();
    
    if (!goal) return jsonResponse({ error: 'Goal is required' }, 400);
    if (!hack) return jsonResponse({ error: 'Hack text is required' }, 400);
...
/**
 * Remove a hack from a specific goal
 */
async function handleRemoveGoalHack(request, env) {
  try {
    ...
    if (!goal) return jsonResponse({ error: 'Goal is required' }, 400);
    if (index === undefined || index < 0) return jsonResponse({ error: 'Valid index is required' }, 400);
    
    const raw = await env.page_content.get('goal_hacks');
    const hacks = raw ? JSON.parse(raw) : getDefaultGoalHacks();
    
    if (hacks[goal] && hacks[goal].length > index) {
