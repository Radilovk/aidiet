...
/**
 * Send an email via MailChannels HTTP API (no SMTP credentials needed).
 * Requires an SPF record in DNS: v=spf1 include:relay.mailchannels.net ...
 * The MAIL_FROM env var sets the sender address (defaults to info@biocode.online).
 */
async function sendEmailViaMailChannels(env, to, subject, htmlBody) {
  if (!to) {
  ...
  const payload = {
    ...
    from: { email: fromEmail, name: 'AiDiet' },
    subject,
  ...
  console.log(`[Email] Sent successfully to ${to} via MailChannels`);
}
  ...
  try {
    ...
    if (clientEmail) {
      const emailPromise = (async () => {
        ...
        await sendEmailViaMailChannels(env, clientEmail, subject, buildPlanReadyEmailHtml(clientName, tpl));
...
async function handleTestSendEmail(request, env) {
  try {
    ...
    await sendEmailViaMailChannels(env, to, subject, buildPlanReadyEmailHtml(name, tpl));
