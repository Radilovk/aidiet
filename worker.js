...
async function handleAnalyzeKidsFoodImage(request, env) {
  try {
    const body = await request.json();
    const { imageData, mimeType, ageGroup, userRole } = body;
    ...
    // Age-specific dietary strategy priorities
    const ageStrategyMap = {
      '0-1': `ПРИОРИТЕТНИ ДИЕТОЛОГИЧНИ ЦЕЛИ ЗА 0-1 г.:
1. БЕЗОПАСНОСТ ОТ ЗАДАВЯНЕ — абсолютен приоритет (консистенция, размер, форма)
2. АЛЕРГЕННА БЕЗОПАСНОСТ — въвеждане на нови храни по правилото 3-5 дни наблюдение
3. ТОКСИЧНОСТ — мед (ботулизъм), нитрати в зеленчуци, тежки метали
4. ХРАНИТЕЛЕН БАЛАНС — желязо, цинк, витамин D, DHA за мозъчно развитие
5. ОПАСНИ ВЕЩЕСТВА — нулева толерантност към добавки, консерванти, оцветители`,
      '1-3': `ПРИОРИТЕТНИ ДИЕТОЛОГИЧНИ ЦЕЛИ ЗА 1-3 г.:
1. БЕЗОПАСНОСТ ОТ ЗАДАВЯНЕ — все още висок риск (цели ядки, грозде, бонбони, твърди плодове)
2. ОПАСНИ ВЕЩЕСТВА — нулева толерантност към изкуствени оцветители, консерванти, овкусители
3. ЗАХАР И СОЛ — минимални количества, формиране на хранителни навици
4. АЛЕРГЕНИ — продължаващ мониторинг
5. ХРАНИТЕЛЕН БАЛАНС — калций, желязо, витамин D, цинк за растеж и имунитет`,
      '3-7': `ПРИОРИТЕТНИ ДИЕТОЛОГИЧНИ ЦЕЛИ ЗА 3-7 г.:
1. ОПАСНИ ВЕЩЕСТВА — консерванти, изкуствени оцветители (E102,E110,E122,E124,E129,E133), овкусители, подсладители — свързани с хиперактивност и алергии
2. ЗАХАР — ограничаване на свободна захар до <25г/ден, скрита захар в сокове и закуски
3. ПРЕРАБОТКА — ултрапреработени храни водят до детско затлъстяване
4. ХРАНИТЕЛНИ НАВИЦИ — формиране на здравословни предпочитания
5. ХРАНИТЕЛЕН БАЛАНС — калций за кости, желязо, витамини A, C, D`,
      '7-12': `ПРИОРИТЕТНИ ДИЕТОЛОГИЧНИ ЦЕЛИ ЗА 7-12 г.:
1. ОПАСНИ ВЕЩЕСТВА — оцветители, консерванти (натриев бензоат, BHA, BHT), овкусители — влияят на концентрация и поведение
2. ЗАХАР И ПРЕРАБОТКА — ограничаване на ултрапреработени храни и бързо хранене
3. КАЛЦИЙ И ВИТАМИН D — критични за костна плътност в периода на растеж
4. ЖЕЛЯЗО — особено при момичета в началото на пубертета
5. КОФЕИН — пълна забрана, влияе на съня и развитието
6. ПОРЦИОНЕН КОНТРОЛ — предотвратяване на детско затлъстяване`,
      '12-16': `ПРИОРИТЕТНИ ДИЕТОЛОГИЧНИ ЦЕЛИ ЗА 12-16 г.:
1. ОПАСНИ ВЕЩЕСТВА — оцветители, консерванти, овкусители, изкуствени подсладители (аспартам, сукралоза)
2. КАЛЦИЙ И ПРОТЕИН — пиков растеж, костна минерализация
3. ЖЕЛЯЗО — критично при момичета (менструация)
4. ХРАНИТЕЛНИ РАЗСТРОЙСТВА — внимание към крайни диети, анорексия, булимия
5. КОФЕИН И ЕНЕРГИЙНИ НАПИТКИ — опасни за сърдечно-съдовата система при подрастващи
6. МАЗНИНИ — транс-мазнини и наситени мазнини в бърза храна`,
      '16-18': `ПРИОРИТЕТНИ ДИЕТОЛОГИЧНИ ЦЕЛИ ЗА 16-18 г.:
1. ОПАСНИ ВЕЩЕСТВА — консерванти, оцветители, овкусители, изкуствени добавки — дългосрочен риск
2. БАЛАНСИРАНО ХРАНЕНЕ — достатъчно протеин, сложни въглехидрати, здравословни мазнини
3. ЖЕЛЯЗО И КАЛЦИЙ — все още необходими за завършване на растежа
4. КОФЕИН — ограничен до <100мг/ден
5. АЛКОХОЛ — абсолютна забрана
6. ХРАНИТЕЛНИ РАЗСТРОЙСТВА — повишен риск в тази възраст`
    };

    const ageStrategy = ageStrategyMap[ageLabel] || ageStrategyMap['3-7'];

    // Choking risk criteria only for children up to 3 years
    const includeChokingRisk = (ageLabel === '0-1' || ageLabel === '1-3');
    const chokingInstruction = includeChokingRisk
      ? '\n- ЗАДЪЛЖИТЕЛНО включи категория "Задавяне" в risks — оцени риска от задавяне (консистенция, размер, форма на храната)'
      : '';
    ...
    const healthRisksInstruction = `
- Ако safetyScore е ПОД 3, ЗАДЪЛЖИТЕЛНО попълни "healthRisks" масива с конкретни заболявания/състояния, свързани с честа консумация на такъв тип храна, с приблизителен процент вероятност за възникване при продължителна консумация
- Ако safetyScore е 3 или повече, "healthRisks" може да е празен масив`;
...
КРИТЕРИИ ЗА ОЦЕНКА (по приоритет за тази възрастова група):
${includeChokingRisk ? '1. БЕЗОПАСНОСТ ОТ ЗАДАВЯНЕ: Риск от задавяне — консистенция, размер, форма на храната\n2. АЛЕРГЕНИ: Потенциални алергени за деца' : '1. АЛЕРГЕНИ: Потенциални алергени за деца'}
${includeChokingRisk ? '3' : '2'}. ОПАСНИ ВЕЩЕСТВА: Изкуствени оцветители (E102, E110, E122, E124, E129, E133 и др.), консерванти (натриев бензоат E211, калиев сорбат E202, нитрити E250, BHA E320, BHT E321), овкусители (MSG E621, изкуствени аромати), изкуствени подсладители (аспартам E951, сукралоза E955, ацесулфам К E950)
${includeChokingRisk ? '4' : '3'}. ЗАХАР: Добавена захар, скрита захар, подсладители
${includeChokingRisk ? '5' : '4'}. СОЛ/НАТРИЙ: Твърде много сол за детски организъм
${includeChokingRisk ? '6' : '5'}. КОФЕИН: ${ageLabel === '0-1' || ageLabel === '1-3' || ageLabel === '3-7' || ageLabel === '7-12' ? 'Абсолютно забранен' : 'Ограничен'} за деца на тази възраст
${includeChokingRisk ? '7' : '6'}. МАЗНИНИ: Транс-мазнини, наситени мазнини, пържени храни
${includeChokingRisk ? '8' : '7'}. ПРЕРАБОТКА: Степен на преработка (ултрапреработени храни — NOVA класификация)
${includeChokingRisk ? '9' : '8'}. ХРАНИТЕЛНА СТОЙНОСТ: Калории, протеин, витамини, минерали подходящи за възрастта
${includeChokingRisk ? '10' : '9'}. ПОРЦИЯ: Подходящ ли е размерът за дете от тази възраст
...
Върни САМО валиден JSON обект (без markdown, без \`\`\`):
{
  ...
  "ageNote": "Специфична бележка за тази възрастова група — какво трябва да знае ${isParent ? 'родителят' : 'детето/тийнейджърът'}",
...
- Рисковете (risks) трябва да покриват: ${riskCategories} като минимум${chokingInstruction}
- ЗАДЪЛЖИТЕЛНО попълни "dangerousSubstances" — списък с всички открити консерванти, оцветители, овкусители, изкуствени подсладители и други опасни добавки. Ако няма такива, върни празен масив []${healthRisksInstruction}
