...
async function generateChatPrompt(env, userMessage, userData, userPlan, conversationHistory, mode = 'consultation') {
  ...
  // Extract psychological profile and communication style from plan
  const psychoProfile = userPlan?.analysis?.psychoProfile || null;
  const communicationStyle = userPlan?.strategy?.communicationStyle || null;
  
  // Build psychological profile context
  let psychoContext = '';
  if (psychoProfile && psychoProfile.temperament && psychoProfile.probability >= 80) {
    psychoContext = `
═══ ПСИХОЛОГИЧЕСКИ ПРОФИЛ НА КЛИЕНТА ═══
Темперамент: ${psychoProfile.temperament} (${psychoProfile.probability}% вероятност)
Обосновка: ${psychoProfile.reasoning || 'Определен от анализа'}
`;
    
    if (communicationStyle) {
      psychoContext += `
СТИЛ НА КОМУНИКАЦИЯ (от Стратегия):
- Тон: ${communicationStyle.tone || 'Професионален и подкрепящ'}
- Подход: ${communicationStyle.approach || 'Индивидуализиран'}
- Насоки: ${communicationStyle.chatGuidelines || 'Адаптирай според темперамента'}

ВАЖНО: Адаптирай комуникацията според темперамента:
`;
      
      // Add temperament-specific guidelines
      switch(psychoProfile.temperament) {
        case 'Холерик':
          psychoContext += `* Бъди ДИРЕКТЕН, фокусиран на РЕЗУЛТАТИ, КРАТКИ обяснения
* Избягвай дълги обосновки - давай конкретни факти и действия
* Подчертавай ПОСТИЖЕНИЯ и прогрес към целите`;
          break;
        case 'Сангвиник':
          psychoContext += `* Бъди ПОЗИТИВЕН, ВДЪХНОВЯВАЩ, подчертавай РАЗНООБРАЗИЕТО
* Използвай ентусиазъм и оптимизъм в отговорите
* Прави храненето интересно и забавно`;
          break;
        case 'Флегматик':
          psychoContext += `* Бъди СПОКОЕН, ПОСТЕПЕНЕН, БЕЗ НАТИСК
* Избягвай драматични промени - предлагай малки стъпки
* Подчертавай сигурността и стабилността на подхода`;
          break;
        case 'Меланхолик':
          psychoContext += `* Бъди ДЕТАЙЛЕН, НАУЧНО ОБОСНОВАН, ЕМПАТИЧЕН
* Давай пълни обяснения с научни факти
* Показвай разбиране към притесненията и съмненията`;
          break;
      }
      psychoContext += '\n═══════════════════════════════════════\n';
    }
...
async function getChatPrompts(env) {
  ...
  // Default prompts
  // NOTE: Communication is adapted based on client's psychological profile from Step 1 analysis
  // The temperament (Холерик/Сангвиник/Флегматик/Меланхолик) influences tone and style
  const prompts = {
...
8. АДАПТИРАЙ тона и стила според психологическия профил и темперамента на клиента (ако е посочен в контекста).
...
ВАЖНИ ПРАВИЛА:
1. Бъди КРАТЪК но информативен, ЯСЕН и директен.
...
6. АДАПТИРАЙ тона според психологическия профил и темперамента на клиента (ако е посочен в контекста).

2. Когато клиентът иска промяна:
   - Анализирай дали е здравословно за цел: {goal}, като вземеш предвид всички фактори (медицински, психологически, метаболитни)
...
function getDefaultPromptTemplates() {
  ...
  return {
    analysis: `═══ ЗАБЕЛЕЖКА: АКТУАЛЕН ПРОМПТ ═══
ТОВА Е ОПРОСТЕНА ВЕРСИЯ НА АКТУАЛНИЯ ПРОМПТ ЗА ВИЗУАЛИЗАЦИЯ В АДМИН ПАНЕЛА.

Реалният промпт в производство (worker.js, функция generateAnalysisPrompt):
- Съдържа АВТОМАТИЧНО ИЗЧИСЛЕНИ стойности (BMR, TDEE, активност скор и др.)
- Използва JavaScript template literals \${variable} вместо {variable}
- Включва ДИНАМИЧНИ секции според данните (напр. допълнителни бележки)
- Вмъква РЕФЕРЕНТНИ ИЗЧИСЛЕНИЯ от backend (Issues #2, #7, #9, #10, #28)

Тази версия показва СТРУКТУРАТА и ЛОГИКАТА на промпта, но не всички динамични елементи.
Променете само ако искате да добавите/премахнете секции или инструкции.
═══════════════════════════════════════════════
...
Бъди КОНКРЕТЕН за {name}. Избягвай общи фрази като "добър метаболизъм" - обясни ЗАЩО и КАК!`,
    ...
    strategy: `═══ ЗАБЕЛЕЖКА: АКТУАЛЕН ПРОМПТ ═══
ТОВА Е ОПРОСТЕНА ВЕРСИЯ НА АКТУАЛНИЯ ПРОМПТ ЗА ВИЗУАЛИЗАЦИЯ В АДМИН ПАНЕЛА.

Реалният промпт в производство (worker.js, функция generateStrategyPrompt):
- Получава данни от анализа в променлива analysis
- Включва ВСИЧКИ полета от анализа (не само {bmr}, {tdee} и др.)
- Използва JavaScript template literals \${analysis.field} и \${data.field}
- Съдържа ДИНАМИЧНИ секции според данните на клиента

Променете само ако искате да промените структурата или инструкциите за стратегията.
═══════════════════════════════════════════════

Базирайки се на здравословния профил и анализа, определи оптималната диетична стратегия:
...
- Вземи предвид уникалната комбинация от: {age} год. {gender}, {goal}, {medicalConditions}, {sportActivity}, стрес: {stressLevel}`,
    ...
    meal_plan: `═══ ЗАБЕЛЕЖКА: АКТУАЛЕН ПРОМПТ ═══
ТОВА Е ОПРОСТЕНА ВЕРСИЯ НА АКТУАЛНИЯ ПРОМПТ ЗА ВИЗУАЛИЗАЦИЯ В АДМИН ПАНЕЛА.

Реалният промпт в производство (worker.js, функция generateMealPlanPrompt):
- Генерира хранения на порции (3 дни наведнъж) за по-добра производителност
- Получава данни от analysis и strategy обекти
- Включва ВСИЧКИ изчисления за калории и макроси за всяко хранене
- Използва ДИНАМИЧНА логика за часове на хранене според хронотип
- Съдържа сложна логика за избор на храни според:
  * Диетични предпочитания и непоносимости
  * Медицински състояния и лекарства
  * Психологически профил и навици
  * Сезонна наличност на продукти

Променете само ако искате да промените структурата или инструкциите за хранителния план.
═══════════════════════════════════════════════
...
Генерирай дни {startDay}-{endDay} с балансирани ястия в правилен хронологичен ред. ЗАДЪЛЖИТЕЛНО включи dailyTotals за проверка!`,
    ...
    summary: `═══ ЗАБЕЛЕЖКА: АКТУАЛЕН ПРОМПТ ═══
ТОВА Е ОПРОСТЕНА ВЕРСИЯ НА АКТУАЛНИЯ ПРОМПТ ЗА ВИЗУАЛИЗАЦИЯ В АДМИН ПАНЕЛА.

Реалният промпт в производство (worker.js, функция generateSummaryPrompt):
- Получава завършения weekPlan с ВСИЧКИ хранения за 7 дни
- Включва изчислените средни стойности за калории и макроси
- Използва данни от strategy обекта
- Генерира финален summary с препоръки, забранени храни, психология и добавки

Променете само ако искате да промените структурата или инструкциите за обобщението.
═══════════════════════════════════════════════
...
ВАЖНО: recommendations/forbidden=САМО конкретни храни според цел {goal}, НЕ общи съвети.`,
    ...
    consultation: `═══ ЗАБЕЛЕЖКА: АКТУАЛЕН ПРОМПТ ═══
ТОВА Е АКТУАЛНИЯТ ПРОМПТ ЗА ЧАТ КОНСУЛТАЦИЯ.

Реалният промпт в производство (worker.js, функция getChatPrompts):
- Използва СЪЩАТА структура и правила както тук
- Може да бъде презаписан от KV storage (admin_consultation_prompt)
- Включва ПЪЛЕН контекст с userData и userPlan при всяка комуникация
- АВТОМАТИЧНО добавя психологически профил и темперамент от Стъпка 1 анализ
- Адаптира комуникацията според темперамента (Холерик/Сангвиник/Флегматик/Меланхолик)

Тази версия е актуалната и се използва директно в chat комуникацията.
═══════════════════════════════════════════════
...
- "Можеш да замениш рибата с пилешко - и двете са отлични източници на протеин. За промяна, активирай режима за промяна."`,
    ...
    modification: `═══ ЗАБЕЛЕЖКА: АКТУАЛЕН ПРОМПТ ═══
ТОВА Е АКТУАЛНИЯТ ПРОМПТ ЗА ЧАТ ПРОМЯНА НА ПЛАНА.

Реалният промпт в производство (worker.js, функция getChatPrompts):
- Използва СЪЩАТА структура и правила както тук
- Може да бъде презаписан от KV storage (admin_modification_prompt)
- Включва ПЪЛЕН контекст с userData и userPlan при всяка комуникация
- АВТОМАТИЧНО добавя психологически профил и темперамент от Стъпка 1 анализ
- Адаптира комуникацията според темперамента (Холерик/Сангвиник/Флегматик/Меланхолик)
- Може да генерира [REGENERATE_PLAN:...] инструкции за промяна на плана
...
- АКО клиентът вече потвърди, НЕ питай отново - ПРИЛОЖИ ВЕДНАГА!`,
    ...
    correction: `═══ ЗАБЕЛЕЖКА: АКТУАЛЕН ПРОМПТ ═══
ТОВА Е ОПРОСТЕНА ВЕРСИЯ НА АКТУАЛНИЯ ПРОМПТ ЗА ВИЗУАЛИЗАЦИЯ В АДМИН ПАНЕЛА.

Реалният промпт в производство (worker.js, функция generateCorrectionPrompt):
- Използва се при неуспешна валидация на генериран план
- Получава СПИСЪК със специфични грешки за коригиране
- Включва пълен JSON на плана с проблеми
- Съдържа всички клиентски данни за контекст
- Автоматично се извиква до 3 пъти при нужда от корекция

Променете само ако искате да промените инструкциите за коригиране на грешки.
═══════════════════════════════════════════════

Ти си експертен диетолог и трябва да КОРИГИРАШ хранителен план, който има следните проблеми:
