...
// Default prompt templates - single source of truth for all AI prompts
// These are used by both generation functions and handleGetDefaultPrompt
...
- АКО клиентът вече потвърди, НЕ питай отново - ПРИЛОЖИ ВЕДНАГА!`,
  ...
  // Analysis prompt generator - used by generateAnalysisPrompt
  analysis: (data) => {
...
Идентифицирай уникален метаболитен профил базиран на:
   ...
   - История: ${data.dietHistory === 'Да' ? `${data.dietType} → ${data.dietResult}` : 'няма'}
...
КРИТИЧНО ВАЖНО - AI МОДЕЛ ЗАДЪЛЖИТЕЛНО ТРЯБВА ДА АНАЛИЗИРА ЛЕКАРСТВА:
   ...
   - Лекарства: ${data.medications} / ${data.medicationsDetails || 'няма'}
...
Бъди КОНКРЕТЕН за ${data.name}. Избягвай общи фрази като "добър метаболизъм" - обясни ЗАЩО и КАК!`;
  ...
  // Strategy prompt generator - used by generateStrategyPrompt
...
Върни JSON със стратегия (БЕЗ универсални препоръки):
{
  ...
  "supplementRecommendations": [
    "! ИНДИВИДУАЛНА добавка 1 за ${data.name} - конкретна добавка с дозировка и обосновка защо Е НУЖНА за този клиент (БАЗИРАНА на: възраст ${data.age} год., пол ${data.gender}, цел ${data.goal}, медицински състояния ${JSON.stringify(data.medicalConditions || [])})",
    ...
    "! ИНДИВИДУАЛНА добавка 3 за ${data.name} - конкретна добавка с дозировка и обосновка защо Е НУЖНА за този клиент (БАЗИРАНА на: хранителни навици ${JSON.stringify(data.eatingHabits || [])}, предпочитания ${JSON.stringify(data.dietPreference || [])})"
  ],
...
- Вземи предвид уникалната комбинация от: ${data.age} год. ${data.gender}, ${data.goal}, ${JSON.stringify(data.medicalConditions || [])}, ${data.sportActivity}, стрес: ${data.stressLevel}`;
  },
...
Генерирай дни ${startDay}-${endDay} с балансирани ястия в правилен хронологичен ред. ЗАДЪЛЖИТЕЛНО включи dailyTotals за проверка!`;
  ...
  // Summary prompt generator - used by generateMealPlanSummaryPrompt
  summary: (data, bmr, recommendedCalories, avgCalories, avgProtein, avgCarbs, avgFats, strategyCompact) => {
...
СТРАТЕГИЯ (КОМПАКТНА):
- Психологическа подкрепа: ${(strategyCompact.psychologicalSupport || []).slice(0, 3).join('; ')}
- Добавки: ${(strategyCompact.supplementRecommendations || []).slice(0, 3).join('; ')}
- Хидратация: ${strategyCompact.hydrationStrategy || 'Минимум 2-2.5л вода дневно'}
- Включвай: ${(strategyCompact.foodsToInclude || []).slice(0, 5).join(', ')}
- Избягвай: ${(strategyCompact.foodsToAvoid || []).slice(0, 5).join(', ')}
...
JSON ФОРМАТ (КРИТИЧНО - използвай САМО числа за числови полета):
{
  ...
  "psychology": ${strategyCompact.psychologicalSupport ? JSON.stringify(strategyCompact.psychologicalSupport) : '["съвет 1", "съвет 2", "съвет 3"]'},
  "waterIntake": "${strategyCompact.hydrationStrategy || 'Минимум 2-2.5л вода дневно'}",
  "supplements": ${strategyCompact.supplementRecommendations ? JSON.stringify(strategyCompact.supplementRecommendations) : '["добавка 1 с дозировка", "добавка 2 с дозировка", "добавка 3 с дозировка"]'}
}
...
/**
 * Progressive meal plan generation: generates meal plan in chunks to avoid token limits
 * Orchestrates the generation of multiple days across several AI requests
 */
async function generateMealPlanProgressive(env, data, analysis, strategy) {
  ...
  const tdee = calculateTDEE(bmr, activityData.combinedScore);
  const deficitData = calculateSafeDeficit(tdee, data.goal);
  const recommendedCalories = deficitData.targetCalories;
  
  // Initialize week plan structure
  const weekPlan = {};
  const previousDays = [];
  
  // Calculate how many chunks we need (7 days total)
  const totalDays = 7;
  const chunksNeeded = Math.ceil(totalDays / DAYS_PER_CHUNK);
  
  console.log(`Progressive generation: Will generate ${totalDays} days in ${chunksNeeded} chunks (${DAYS_PER_CHUNK} days per chunk)`);
  ...
  for (let chunkIndex = 0; chunkIndex < chunksNeeded; chunkIndex++) {
    ...
    console.log(`Progressive generation: Generating chunk ${chunkIndex + 1}/${chunksNeeded} (days ${startDay}-${endDay})`);
    ...
    try {
      ...
      // Parse the response
      const chunkPlan = parseAIResponse(chunkResponse);
      
      if (!chunkPlan || chunkPlan.error) {
        const errorMsg = chunkPlan?.error || 'Невалиден формат на отговор';
        console.error(`Chunk ${chunkIndex + 1} parsing failed:`, errorMsg);
        throw new Error(`Парче ${chunkIndex + 1} (Дни ${startDay}-${endDay}): ${errorMsg}`);
      }
      
      // Merge chunk into week plan
      for (let day = startDay; day <= endDay; day++) {
        const dayKey = `day${day}`;
        if (chunkPlan[dayKey]) {
          weekPlan[dayKey] = chunkPlan[dayKey];
          previousDays.push({
            day: day,
            meals: chunkPlan[dayKey].meals || []
          });
          console.log(`Progressive generation: Day ${day} added to plan (${chunkPlan[dayKey].meals?.length || 0} meals)`);
        } else {
          console.warn(`Progressive generation: Day ${day} missing in chunk response`);
        }
      }
    } catch (error) {
      console.error(`Progressive generation: Chunk ${chunkIndex + 1} failed:`, error);
      throw new Error(`Парче ${chunkIndex + 1} (Дни ${startDay}-${endDay}): ${error.message}`);
    }
  }
  
  console.log(`Progressive generation: All chunks complete. Generated ${Object.keys(weekPlan).length} days`);
...
/**
 * Helper function to fetch and format dynamic food lists from KV storage
 * Returns sections for whitelist and blacklist to be included in prompts
 */
async function getDynamicFoodListsSections(env) {
  ...
  try {
    if (env && env.page_content) {
      // Fetch whitelist from KV
      const whitelistData = await env.page_content.get('food_whitelist');
      if (whitelistData) {
        const whitelist = JSON.parse(whitelistData);
        if (whitelist && whitelist.length > 0) {
          dynamicWhitelistSection = `\n\n=== ДИНАМИЧЕН WHITELIST (приоритетни храни) ===\n${whitelist.join(', ')}\nВАЖНО: Включвай тези храни колкото е възможно!`;
        }
      }
      
      // Fetch blacklist from KV
      const blacklistData = await env.page_content.get('food_blacklist');
      if (blacklistData) {
        const blacklist = JSON.parse(blacklistData);
        if (blacklist && blacklist.length > 0) {
          dynamicBlacklistSection = `\n\n=== ДИНАМИЧЕН BLACKLIST (забранени храни) ===\n${blacklist.join(', ')}\nКРИТИЧНО: НИКОГА не използвай тези храни!`;
        }
      }
    }
  } catch (error) {
    console.error('Error fetching dynamic food lists:', error);
    // Continue without dynamic lists if there's an error
  }
...
async function generateAnalysisPrompt(data, env) {
  ...
  // Use DEFAULT_PROMPTS.analysis function
  return DEFAULT_PROMPTS.analysis(data);
}
...
async function generateStrategyPrompt(data, analysis, env) {
  ...
  // Use DEFAULT_PROMPTS.strategy function
  return DEFAULT_PROMPTS.strategy(data, analysisCompact);
}
...
`;
  ...
  // If custom prompt exists, use it; otherwise use DEFAULT_PROMPTS.mealPlan
  if (customPrompt) {
    return replacePromptVariables(customPrompt, {
      userData: data,
...
async function generateMealPlanSummaryPrompt(data, analysis, strategy, bmr, recommendedCalories, weekPlan, env) {
  ...
  // Build compact strategy for prompt
  const strategyCompact = {
  ...
  // Use DEFAULT_PROMPTS.summary function
  return DEFAULT_PROMPTS.summary(data, bmr, recommendedCalories, avgCalories, avgProtein, avgCarbs, avgFats, strategyCompact);
}
...
async function getChatPrompts(env) {
  ...
  // Use DEFAULT_PROMPTS as the base
  const prompts = {
    consultation: DEFAULT_PROMPTS.consultation,
    modification: DEFAULT_PROMPTS.modification
  };
...
async function handleGetDefaultPrompt(request, env) {
  try {
    ...
    // For consultation and modification, return from DEFAULT_PROMPTS directly (they're simple strings)
    if (type === 'consultation') {
      return jsonResponse({ success: true, prompt: DEFAULT_PROMPTS.consultation });
    }
