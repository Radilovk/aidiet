...
function buildCombinedAdditionalNotes(data) {
  ...
  if (baseNotes && specificAnswers) {
    return `${baseNotes}\n\n[Специфични данни за клиничен протокол]\n${specificAnswers}`;
  }
  if (specificAnswers) {
    return `[Специфични данни за клиничен протокол]\n${specificAnswers}`;
  }
  return baseNotes;
}
...
7. Разнообразие: Различни ястия от предишните дни${data.eatingHabits && data.eatingHabits.includes('Не закусвам') ? '\n8. ВАЖНО: Клиентът НЕ ЗАКУСВА - без "Закуска". ЗАДЪЛЖИТЕЛНО минимум 3 хранения дневно: Обяд + Следобедна закуска + Вечеря (или Обяд + Вечеря + Късна закуска). НЕ ограничавай до само 2 хранения!' : ''}${sweetsCravingRule}${buildFreeMealInstruction(strategy, startDay, endDay)}
...
3. СПЕЦИАЛНИ СЛУЧАИ:
   a) Ако клиентът НЕ ЗАКУСВА:
      ...
      - ЗАДЪЛЖИТЕЛНО: дори без закуска, общият брой хранения трябва да е минимум 3 (напр. Обяд + Следобедна закуска + Вечеря).
б) План с по-малко от 3 хранения дневно да се избягва. Той е допустим само и единствено, ако има реална причина за това.
   ...
   b) СВОБОДНО ХРАНЕНЕ/ЛЮБИМА ХРАНА:
      ...
      - ВАЖНО: Дори в свободния ден — минимум 3 хранения! Ако клиентът не закусва, добави Следобедна закуска преди вечерята.
