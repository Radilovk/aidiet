...
async function generateChatPrompt(env, userMessage, userData, userPlan, conversationHistory, mode = 'consultation') {
  ...
  // IMPORTANT: Role definition aligned with Step 1 analysis for consistency
  // Step 1 establishes professional clinical expertise, which should be maintained in chat
  const baseContext = `Ти си професионален клиничен диетолог, психолог и здравен асистент за ${userData.name}.
Твоята роля е да използваш холистичен подход, анализирайки корелациите между всички фактори (сън, стрес, хранене, медицински състояния) за индивидуализирани препоръки.
...
async function getChatPrompts(env) {
  ...
  // Default prompts
  // NOTE: These prompts are aligned with Step 1 analysis communication guidelines
  // Step 1 establishes professional clinical expertise and holistic approach
  const prompts = {
    consultation: `ТЕКУЩ РЕЖИМ: КОНСУЛТАЦИЯ

ТВОЯ РОЛЯ (от Стъпка 1 - Анализ):
Ти си професионален клиничен диетолог, психолог и здравен асистент. Използваш холистичен подход, 
анализирайки корелациите между всички фактори: сън, стрес, хранене, медицински състояния, 
психологически профил. Твоите отговори са базирани на научни принципи и индивидуализирани за клиента.
...
5. Винаги поддържай мотивиращ, топъл и подкрепящ тон.
6. Форматирай отговорите си ясно - използвай нови редове за разделяне на мисли.
7. Задавай максимум 1 въпрос на отговор.
8. Обяснявай ЗАЩО и КАК, не просто "добре" или "лошо" - дай контекст според профила.
...
ТВОЯ РОЛЯ (от Стъпка 1 - Анализ):
Ти си професионален клиничен диетолог, психолог и здравен асистент. Използваш холистичен подход при оценка на промените.
...
Реалният промпт в производство (worker.js, функция generateAnalysisPrompt, ред 3508):
...
Реалният промпт в производство (worker.js, функция generateStrategyPrompt, ред 3860):
...
Реалният промпт в производство (worker.js, функция generateMealPlanPrompt, ред 1467):
...
Реалният промпт в производство (worker.js, функция getChatPrompts, ред 4586):
...
- Следва насоките от Стъпка 1 за професионален холистичен подход
...
Реалният промпт в производство (worker.js, функция getChatPrompts, ред 4606):
- Използва СЪЩАТА структура и правила както тук
