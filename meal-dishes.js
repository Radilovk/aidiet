 ...
 *               Планът мащабира ястието спрямо целта на храненето, но пази
 *               пропорцията ти — марулята не става носеща съставка.
 ...
 *
 * Грамажите в готовия план се закръглят: под 50 г на 5 г, от 50 г нагоре на
 * 50 г. Тук пиши реалната порция — закръглянето е работа на плана.
 *
...
function dish(id, name, products, timing, opts = {}) {
  const totalGrams = products.reduce((sum, [, grams]) => sum + grams, 0) || 1;
  return {
    id,
    name,
    products: products.map(([product, grams]) => ({
      name: product,
      grams,
      // Делът се извежда от грамажите — един източник на истина за формата.
      share: grams / totalGrams,
    })),
...
export const MEAL_DISHES = [
  ...
  dish('meal_oatmeal_banana_nuts', 'Овесена каша с банан и орехи', [['овесени ядки', 80], ['мляко', 250], ['банан', 100], ['орехи', 20]],
    ['breakfast'], { vegetarian: true, universality: 5 }),
  ...
  dish('meal_eggs_potato_tomato', 'Яйца с картофи и домати', [['яйца', 150], ['картофи', 200], ['Домати', 80]],
    ['breakfast', 'main'], { vegetarian: true }),
  dish('meal_yogurt_oats_fruit_nuts', 'Кисело мляко с овес, плодове и орехи', [['кисело мляко', 250], ['овесени ядки', 60], ['банан', 80], ['орехи', 20]],
    ['breakfast'], { vegetarian: true, universality: 5 }),
