...
1. **Автоматично откриване на данни**
   - Страницата автоматично търси всички записи с ключ `questionnaire2_*` в localStorage
   - Показва намерените данни с основна информация (име, имейл, дата и др.)

2. **Запазване към сървъра**
   - Бутон "Запази в сървъра" изпраща данните към API endpoint `/api/save-questionnaire2-data`
   - Данните се съхраняват в KV хранилището с ключ `questionnaire2_data:{clientId}`
   - След успешно запазване, данните се изтриват от localStorage

3. **Изтегляне като файл**
   - Бутон "Изтегли като файл" позволява download на данните като JSON файл
   - Файлът може да бъде архивиран или обработен офлайн

4. **Визуализация на данни**
   - Показва ID, дата и час на попълване
   - Основна информация: име, имейл, възраст, пол
   - Брой прикачени файлове (ако има такива)
...
3. Кликнете "Запази в сървъра" за да изпратите данните към сървъра и KV хранилището
4. Алтернативно, кликнете "Изтегли като файл" за да запазите данните локално
