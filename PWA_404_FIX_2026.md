...
sw.js          - 16 path references updated + BASE_PATH constant added
PWA_404_FIX_2026.md - Documentation file added
```
...
```javascript
// sw.js
const BASE_PATH = '/aidiet';  // ✅ Configurable constant
const STATIC_CACHE = [
  `${BASE_PATH}/index.html`,           // ✅ Absolute path with constant
  `${BASE_PATH}/icon-192x192.png`,     // ✅ Absolute path with constant
  // ...
];
```
...
**Note**: manifest.json uses hardcoded paths. When changing the base path, update both `BASE_PATH` in sw.js and icon paths in manifest.json.
